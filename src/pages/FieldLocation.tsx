  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div><div className="flex items-center gap-2 text-xs uppercase tracking-wider text-[var(--av-text)]/50"><StatePulse active={active > 0} /> Field operations</div><h1 className="text-3xl font-bold mt-1">Verified field visits</h1><p className="text-sm text-[var(--av-text)]/60 mt-1">Avenize records the claim, time, location evidence and verification result without covert continuous tracking.</p></div>
        <div className="flex items-center gap-3">{online ? <Wifi className="text-[var(--av-success)]" size={16}/> : <WifiOff className="text-[var(--av-warning)]" size={16}/>} {queued > 0 && <span className="text-xs px-2 py-1 rounded-full bg-[var(--av-warning-soft)] text-[var(--av-warning)]">{queued} queued</span>}{isManager && <MotionButton onClick={() => setShowCreate(v => !v)} className="px-4 py-2 rounded-lg bg-black text-white flex items-center gap-2"><Plus size={16}/> Assign visit</MotionButton>}</div>
      </div>
      {showCreate && isManager && <MotionCard className="bg-[var(--av-surface)] border border-[var(--av-border)] rounded-2xl p-5 grid md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2 flex justify-end gap-2"><button onClick={()=>setShowCreate(false)} className="px-4 py-2 rounded-lg border">Cancel</button><MotionButton onClick={()=>void createVisit()} className="px-4 py-2 rounded-lg bg-[var(--av-success)] text-white">Create visit</MotionButton></div>
      </MotionCard>}
      ...
      <MotionCard className="bg-[var(--av-surface-elevated)] rounded-2xl border border-[var(--av-border)] overflow-hidden">
        <div className="p-5 border-b border-[var(--av-border)] flex justify-between"><div><h2 className="font-semibold">Visit timeline</h2><p className="text-xs text-[var(--av-text)]/50">Evidence is event-based: arrival and completion, not hidden background surveillance.</p></div><button onClick={()=>void load()} className="p-2 rounded-lg hover:bg-[var(--av-surface-2)]" aria-label="Refresh visits"><RefreshCw size={16}/></button></div>
        {loading ? <div className="p-10 text-center"><RefreshCw className="mx-auto animate-spin"/></div> : visits.length===0 ? <div className="p-10 text-center text-[var(--av-text)]/50"><MapPin className="mx-auto mb-2"/><p>No field visits have been assigned.</p></div> : <div className="divide-y divide-black/5">{visits.map(visit=><div key={visit.id} className="p-5 grid lg:grid-cols-[1fr_auto] gap-4"><div><div className="flex items-center gap-2"><h3 className="font-semibold">{visit.customer_name}</h3><span className="text-xs px-2 py-1 rounded-full bg-[var(--av-surface-2)] capitalize">{visit.status.replace('_',' ')}</span></div><p className="text-sm text-[var(--av-text)]/60 mt-1">{visit.customer_address || 'Customer location not specified'}</p><div className="flex flex-wrap gap-4 text-xs text-[var(--av-text)]/60 mt-3"><span>{visit.scheduled_at ? new Date(visit.scheduled_at).toLocaleString('en-NG',{dateStyle:'medium',timeStyle:'short'}) : 'Unscheduled'}</span><span className="inline-flex items-center gap-1"><ShieldCheck size={13}/>{visit.arrival_verification_status.replace('_',' ')}</span>{visit.arrival_distance_meters != null && <span className="inline-flex items-center gap-1"><Navigation size={13}/>{Math.round(visit.arrival_distance_meters)}m from target</span>}</div></div><div className="flex items-center gap-2 lg:self-center">{!['completed','cancelled'].includes(visit.status) && visit.assigned_staff_id===staff?.id && <MotionButton disabled={busyId===visit.id} onClick={()=>void act(visit,visit.arrived_at ? 'complete_visit' : 'start_visit')} className={`px-4 py-2 rounded-lg text-white ${visit.arrived_at ? 'bg-black' : 'bg-[var(--av-success)]'}`}>{busyId===visit.id ? <RefreshCw size={16} className="animate-spin"/> : visit.arrived_at ? 'Complete visit' : 'I am here'}</MotionButton>}{visit.arrival_verification_status==='verified' && <CheckCircle2 className="text-[var(--av-success)]" size={22}/>}</div></div>)}</div>}
      </MotionCard>
      <div className="p-4 rounded-xl bg-[var(--av-primary-soft)] border border-[var(--av-primary-soft)] text-[var(--av-primary-active)] text-sm flex gap-3"><AlertTriangle size={18} className="shrink-0"/><p>Location evidence is only captured when a salesperson checks in or completes a visit. Avenize records what was claimed, what evidence was available, and the verification result.</p></div>
    </div>
  )
}
function Metric({ title, value }: { title: string; value: string | number }) { return <MotionCard className="bg-[var(--av-surface-elevated)] rounded-xl border border-[var(--av-border)] p-4"><p className="text-xs text-[var(--av-text)]/50">{title}</p><p className="text-2xl font-bold mt-1">{value}</p></MotionCard> }
