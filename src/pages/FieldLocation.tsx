...
type Visit = {
  ...
  arrival_verification_status: string
  completion_distance_meters: number | null
  completion_verification_status: string
  outcome: string | null
  notes: string | null
}
...
async function getPosition() {
  ...
  return new Promise<GeolocationPosition>((resolve, reject) => navigator.geolocation.getCurrentPosition(resolve, reject, { enableHighAccuracy: true, timeout: 10000, maximumAge: 30000 }))
}
...
export default function FieldLocation() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error('field_visits.load', error)
      showToast('Field visits could not be refreshed.', 'error')
  ...
  const act = async (visit: Visit, kind: 'start_visit' | 'complete_visit') => {
    ...
    if (!navigator.onLine) {
      enqueuePresenceAction({ kind, payload, id })
      setQueued(getPresenceQueue().filter(x => x.kind.includes('visit')).length)
      showToast('Visit event saved offline and queued for sync.', 'success')
    ...
    try {
      ...
      if (!navigator.onLine || String(error?.message || '').match(/fetch|network|Failed to/i)) {
        enqueuePresenceAction({ kind, payload, id }); setQueued(getPresenceQueue().filter(x => x.kind.includes('visit')).length)
        showToast('Network interrupted. The field event is queued safely.', 'info')
  ...
  const createVisit = async () => {
    ...
    try {
      const { error } = await supabase.rpc('create_field_visit', {
        ...
        p_lat: form.latitude ? Number(form.latitude) : null,
        p_lng: form.longitude ? Number(form.longitude) : null,
        p_radius_meters: Number(form.radius_meters) || 150,
        p_scheduled_at: form.scheduled_at ? new Date(form.scheduled_at).toISOString() : null,
        p_notes: form.notes || null,
      })
      if (error) throw error
      setShowCreate(false); setForm({ assigned_staff_id:'',customer_name:'',customer_address:'',latitude:'',longitude:'',radius_meters:'150',scheduled_at:'',notes:'' })
      showToast('Field visit assigned.', 'success'); await load()
