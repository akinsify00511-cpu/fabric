...
import {
  ...
  Megaphone, Building, UserRound, Crown, Sparkles,
} from 'lucide-react'
...
const SECTIONS: MoreSection[] = [
  ...
  { title: 'Tools & Productivity', items: [
  ...
  { title: 'Intelligence & Reports', items: [
...
export default function More() {
  ...
  if (params.get('customize') === '1') return <WorkspacePreferences />

  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-semibold text-[var(--av-text)] mb-1">Your workspace</h1>
          <p className="text-sm text-[var(--av-text-secondary)]">Keep the tools you need close. Everything else stays out of the way.</p>
        </div>
        <div className="flex items-center gap-2">
          <Link to="/app/more?customize=1" className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--av-primary)] text-white text-sm font-medium">
            <Sparkles size={15} />
            Customize workspace
          </Link>
          <div className={`px-3 py-2 rounded-lg text-xs font-medium capitalize ${
            plan === 'enterprise' ? 'bg-purple-100 text-purple-700' : plan === 'pro' ? 'bg-[var(--av-primary-soft)] text-[var(--av-primary)]' : plan === 'starter' ? 'bg-blue-100 text-blue-700' : 'bg-[var(--av-surface-3)] text-[var(--av-text-secondary)]'
          }`}>
            {plan} Plan
          </div>
        </div>
      ...
      <div className="mb-8 rounded-2xl border border-[var(--av-border)] bg-[var(--av-surface)] p-5">
        <p className="text-sm font-semibold text-[var(--av-text)]">Built around your role</p>
        <p className="text-sm text-[var(--av-text-secondary)] mt-1">Your organization and role determine what you are allowed to use. You decide what belongs in your everyday workspace.</p>
      </div>
          ...
          <div key={section.title}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {section.items.map((item) => {
                ...
                return isLocked ? (
                  <div key={item.to} className="bg-[var(--av-surface)] rounded-[var(--av-radius-lg)] border border-[var(--av-border)] p-4 flex flex-col gap-2 opacity-60">
                    <div className="flex items-start justify-between">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[var(--av-surface-3)] text-[var(--av-text-secondary)]"><Icon size={18} strokeWidth={2} /></div>
                      <Lock size={14} className="text-[var(--av-text-muted)]" />
                    </div>
                    <div><span className="text-sm font-medium text-[var(--av-text)]">{item.label}</span><p className="text-xs text-[var(--av-text-muted)]">{item.desc}</p></div>
                    <Link to="/upgrade" className="text-xs text-[var(--av-primary)] font-medium">Upgrade to unlock →</Link>
                  </div>
                ) : (
                  <Link key={item.to} to={item.to} className="bg-[var(--av-surface)] rounded-[var(--av-radius-lg)] border border-[var(--av-border)] p-4 flex flex-col gap-2 hover:border-[var(--av-border-strong)] hover:shadow-[var(--av-shadow-sm)] transition">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[var(--av-primary-soft)] text-[var(--av-primary)]"><Icon size={18} strokeWidth={2} /></div>
                    <div><span className="text-sm font-medium text-[var(--av-text)]">{item.label}</span><p className="text-xs text-[var(--av-text-muted)]">{item.desc}</p></div>
                  </Link>
                )
              })}
      ...
      <div className="mt-8 rounded-[var(--av-radius-xl)] bg-gradient-to-r from-[#4285F4] to-[#8B5CF6] p-6 text-white flex items-center gap-4">
        <Crown size={28} />
        <div className="flex-1"><p className="font-semibold">Get more from Avenize</p><p className="text-sm text-white/80">Unlock automations, advanced reports, and every module with Pro.</p></div>
