...
export default function MeetingCapture(){
 ...
 const [captureKind,setCaptureKind]=useState<'note'|'decision'|'action'>('note')
 ...
 const load=useCallback(async()=>{
  ...
  if(meetingId){setDecisions(await fetchMeetingDecisions(meetingId));setActions(await fetchMeetingActions(meetingId))}
 ...
 const saveTypedCapture=async()=>{if(!note.trim()||!meeting||!staff?.business_id)return
  if(captureKind==='note'){void saveCapture('text',note);setNote('');return}
  if(captureKind==='decision'){const id=await saveMeetingDecision(meeting.id,note.trim());if(id){setDecisions(v=>[...v,{id,meeting_id:meeting.id,decision_text:note.trim(),rationale:null,decided_by:staff.id,timestamp_ms:null,status:'decided',created_at:new Date().toISOString()} as MeetingDecision]);setNote('');showToast('Decision recorded','success')}else{showToast('Could not record decision','error')}return}
  const id=await saveMeetingAction(meeting.id,note.trim());if(id){setActions(v=>[...v,{id,meeting_id:meeting.id,decision_id:null,task_id:null,action_text:note.trim(),assignee_id:null,due_date:null,priority:'medium',status:'open',timestamp_ms:null,created_at:new Date().toISOString()} as MeetingAction]);setNote('');showToast('Action item recorded','success')}else{showToast('Could not record action item','error')}}

 const promoteAction=async(a:MeetingAction)=>{const tid=await promoteMeetingActionToTask(a.id,a.action_text,a.assignee_id,a.due_date,a.priority);if(tid){setActions(v=>v.map(x=>x.id===a.id?{...x,status:'in_progress',task_id:tid}:x));showToast('Promoted to a task','success')}else{showToast('Could not create task','error')}}
 ...
 <aside className="bg-white text-black border-l border-black/10 flex flex-col min-h-0"><div className="border-b border-black/5 flex"><button onClick={()=>{setSideTab('chat')}} className={`flex-1 px-4 py-3 text-sm font-semibold flex items-center justify-center gap-2 ${sideTab==='chat'?'text-[var(--av-primary)] border-b-2 border-[var(--av-primary)]':'text-black/50'}`}><MessageSquare size={16}/> Chat{chatUnread>0&&<span className="rounded-full bg-red-600 text-white text-[10px] px-1.5 py-0.5">{chatUnread}</span>}</button><button onClick={()=>{setSideTab('capture')}} className={`flex-1 px-4 py-3 text-sm font-semibold ${sideTab==='capture'?'text-[var(--av-primary)] border-b-2 border-[var(--av-primary)]':'text-black/50'}`}>Capture</button></div>{sideTab==='chat'?(<MeetingChatPanel meetingId={meetingId!} businessId={staff!.business_id} visible={sideTab==='chat'} onUnread={handleChatUnread}/>):(<><div className="p-4 border-b border-black/5"><p className="text-xs text-black/50">Everything captured here stays attached to this meeting.</p></div><div className="p-4 border-b border-black/5 space-y-2"><div className="grid grid-cols-3 gap-1 rounded-xl bg-black/[0.04] p-1">{(['note','decision','action'] as const).map(k=><button key={k} onClick={()=>setCaptureKind(k)} className={`rounded-lg py-1.5 text-xs font-semibold capitalize ${captureKind===k?'bg-white shadow-sm text-[var(--av-primary)]':'text-black/50'}`}>{k==='note'?'Note':k==='decision'?'Decision':'Action item'}</button>)}</div><textarea value={note} onChange={e=>setNote(e.target.value)} rows={3} className="w-full rounded-xl border border-black/10 p-3 text-sm" placeholder={captureKind==='note'?'Capture a customer request or note…':captureKind==='decision'?'Record a decision (what was decided, and why)…':'Record an action item (what must happen next)…'}/><button onClick={()=>{void saveTypedCapture()}} disabled={!note.trim()} className="w-full rounded-xl bg-[var(--av-primary)] text-white py-2.5 text-sm font-semibold disabled:opacity-40"><Send size={15} className="inline mr-2"/>{captureKind==='note'?'Save capture':captureKind==='decision'?'Record decision':'Record action'}</button><div className="grid grid-cols-2 gap-2"><button onClick={voiceListening?stopVoice:startVoice} className={`rounded-xl border border-black/10 py-2 text-sm ${voiceListening?'bg-red-50 text-red-600':''}`}><Mic size={15} className="inline mr-1"/>{voiceListening?'Stop voice':'Voice capture'}</button><label className="rounded-xl border border-black/10 py-2 text-sm text-center cursor-pointer"><Paperclip size={15} className="inline mr-1"/>Attach<input type="file" className="hidden" onChange={e=>{const f=e.target.files?.[0];if(f)void addAttachment(f);e.currentTarget.value=''}}/></label></div>{voiceText&&<div className="rounded-xl bg-black/[0.03] p-3 text-xs text-black/60">{voiceText}</div>}</div><div className="flex-1 overflow-y-auto p-4"><div className="flex items-center justify-between mb-3"><h3 className="text-sm font-semibold">Meeting captures</h3><span className="text-xs text-black/40">{captures.length}</span></div>{captures.length===0?<p className="text-xs text-black/40">No captures yet.</p>:<div className="space-y-2">{captures.map(c=><div key={c.id} className="rounded-xl border border-black/5 bg-[#F8F9FA] p-3"><div className="flex items-center justify-between gap-2"><span className="text-[10px] uppercase tracking-wide text-[var(--av-primary)]">{c.capture_type}</span><span className="text-[10px] text-black/35">{new Date(c.created_at).toLocaleTimeString('en-NG',{hour:'2-digit',minute:'2-digit'})}</span></div><p className="text-sm font-medium mt-1">{c.title||'Capture'}</p>{c.body&&<p className="text-xs text-black/60 mt-1 whitespace-pre-wrap">{c.body}</p>}{c.storage_path&&<p className="text-[10px] text-black/40 mt-1">Attachment stored securely</p>}</div>)}</div>}

{(decisions.length>0||actions.length>0)&&<div className="border-t border-black/5 p-4 space-y-3">
{decisions.length>0&&<div><h3 className="text-sm font-semibold mb-2">Decisions ({decisions.length})</h3><div className="space-y-1.5">{decisions.map(d=><div key={d.id} className="rounded-xl border border-black/5 bg-[#F8F9FA] p-3"><p className="text-sm">{d.decision_text}</p><span className="text-[10px] text-black/40">{new Date(d.created_at).toLocaleTimeString('en-NG',{hour:'2-digit',minute:'2-digit'})}</span></div>)}</div></div>}
{actions.length>0&&<div><h3 className="text-sm font-semibold mb-2">Action items ({actions.length})</h3><div className="space-y-1.5">{actions.map(a=><div key={a.id} className="rounded-xl border border-black/5 bg-[#F8F9FA] p-3"><div className="flex items-start justify-between gap-2"><p className="text-sm flex-1">{a.action_text}</p>{a.task_id?<span className="text-[10px] text-[var(--av-success)] font-semibold whitespace-nowrap">Task created</span>:<button onClick={()=>void promoteAction(a)} className="text-[10px] font-semibold text-[var(--av-primary)] whitespace-nowrap">Promote to task</button>}</div></div>)}</div></div>}
</div>}
</div></>)}</aside></main></div>
}