...
export default function Settings() {
  ...
  const SETTINGS_ITEMS = [
    ...
    { to: '/app/security', icon: Shield, label: 'Security', desc: '2FA, audit log', color: 'bg-red-50 text-[var(--av-danger)]' },
    { to: '/app/sso', icon: Key, label: 'Single Sign-On', desc: 'SAML, OIDC, Okta, Azure', color: 'bg-blue-50 text-[var(--av-primary)]' },
    ...
    { to: '/settings?lang', icon: Globe, label: 'Language', desc: 'i18n, timezone', color: 'bg-green-50 text-[var(--av-success)]' },
  ]

  return (
    <div className="pb-20">
      <h1 className="text-xl font-medium text-[var(--av-text)] mb-6">{tr('settings', 'Settings')}</h1>

      {/* User Info */}
      <div className="bg-[var(--av-surface-elevated)] rounded-2xl border border-[var(--av-border-strong)]/[0.06] p-6 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--av-text)]">{staff?.email}</p>
        ...
        {SETTINGS_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.to}
              to={item.to}
              className="flex items-center gap-3 p-4 rounded-xl border border-[var(--av-border-strong)]/[0.06] hover:bg-[var(--av-surface-3)] transition-colors"
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-[var(--av-text)]">{tr(`setting_${item.label.toLowerCase().replace(/\s+/g, '_')}_desc`, item.desc)}</p>
              </div>
              <ChevronRight size={16} className="text-[var(--av-text)]" />
            </Link>
          )
        })}
      </div>
