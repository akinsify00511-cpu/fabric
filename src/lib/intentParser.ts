// Deterministic natural-language intent parser — the single source of
// truth shared by the parse-intent edge function and the AICapture
// client-side fallback. Pure, testable, no I/O.

export interface EntityMatch { field: string; value: string; raw: string }
export interface ProposedDestination { entity_type: string; action: string; reason: string }
...
const WRITE_ACTIONS = ['mark_won', 'upsert', 'create', 'mark_paid', 'reduce', 'increase', 'draft', 'calculate']
...
export function extractPercent(text: string): string | null {
  const m = text.match(/(\d{1,3})\s?%/)
  return m ? m[1] : null
}

export function extractName(text: string, after: string[]): string | null {
...
export function detectEventType(text: string): { event_type: string; confidence: number } {
...
// Classify the AI action authority rung a parsed intent implies.
// Returns null for read-only captures.
export function classifyRung(intent: ParsedIntent): string | null {
  const impliesWrite = intent.destinations.some(d => WRITE_ACTIONS.includes(d.action))
  if (!impliesWrite) return null
  return intent.needs_confirmation ? 'execute_with_approval' : 'low_risk_execute'
}
