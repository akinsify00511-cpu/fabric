...
export function PageTransition({ children }: { children: ReactNode }) {
  ...
  return <div key={location.pathname} className="av-page-transition" data-page={location.pathname}>{children}</div>
}

export function MotionButton({ children, className = '', ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={`av-motion-button ${className}`}>{children}</button>
}
