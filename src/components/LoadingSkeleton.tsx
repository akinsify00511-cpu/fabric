export default function LoadingSkeleton({ type = 'card', count = 1 }: { type?: 'card' | 'table' | 'detail' | 'list', count?: number }) {
  const SkeletonItem = () => (
    <div className="animate-pulse">
      {type === 'card' && (
        <div className="bg-white rounded-xl border border-white p-6">
        ...
        <div className="bg-white rounded-xl border border-white overflow-hidden">
          <div className="p-4 border-b border-white">
            <div className="flex gap-4">
              ...
              <div className="h-4 bg-white rounded w-24 ml-auto" />
            </div>
          </div>
          {[...Array(5)].map((_, i) => (
            <div key={i} className="p-4 border-b border-white last:border-0">
              <div className="flex gap-4 items-center">
                <div className="w-8 h-8 bg-white rounded-full" />
                <div className="flex-1">
                  <div className="h-3 bg-white rounded w-32 mb-1" />
                  <div className="h-2 bg-white rounded w-48" />
                </div>
                <div className="h-6 bg-white rounded w-20" />
              </div>
            </div>
          ))}
        </div>
      ...
      {type === 'detail' && (
        <div className="bg-white rounded-xl border border-white p-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <div className="h-5 bg-white rounded w-40 mb-2" />
          ...
          <div className="space-y-4">
            {[...Array(4)].map((_, i) => (
              <div key={i}>
                <div className="h-3 bg-white rounded w-20 mb-2" />
      ...
      {type === 'list' && (
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white rounded-lg border border-white p-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white rounded-lg" />
                <div className="flex-1">
                  <div className="h-4 bg-white rounded w-32 mb-1" />
                  <div className="h-3 bg-white rounded w-24" />
                </div>
