...
const ROUTE_MODULE: Record<string, ModuleKey> = {
  ...
  '/app/risks': 'self_audit', '/app/trust': 'self_audit', '/app/review': 'self_audit',
  '/app/chat': 'chat', '/app/live-chat': 'chat', '/app/whatsapp': 'chat', '/app/sms': 'chat',
  '/app/crm': 'crm', '/app/leads': 'crm', '/app/quotes': 'crm',
  '/app/properties': 'crm', '/app/property-owners': 'crm', '/app/property-sales': 'crm',
  '/app/leases': 'crm', '/app/social': 'crm', '/app/sales-performance': 'crm', '/app/signatures': 'legal',
  '/app/finance': 'finance', '/app/payments': 'finance', '/app/payroll': 'finance',
  '/app/budgets': 'finance', '/app/cashflow': 'finance', '/app/accounting': 'finance', '/app/e-invoicing': 'finance',
  '/app/hr': 'hr', '/app/recruitment': 'hr', '/app/appraisals': 'hr', '/app/okrs': 'hr',
  '/app/merit': 'hr', '/app/personas': 'hr', '/app/staff': 'hr',
  '/app/projects': 'projects', '/app/operations': 'projects', '/app/inventory': 'inventory',
  '/app/logistics': 'inventory', '/app/equipment': 'inventory', '/app/lab': 'inventory',
  '/app/maintenance': 'inventory', '/app/infrastructure': 'inventory', '/app/migration': 'inventory',
  '/app/knowledge': 'knowledge', '/app/documents': 'knowledge', '/app/approvals': 'approvals',
...
export default function Shell() {
  ...
  const { selectedTools, loading: selectionLoading } = useWorkspaceSelection(accessibleTools as ToolKey[])
  ...
  const itemVisible = (item: NavItem) => {
    if (loading || modulesLoading || selectionLoading) {
      return item.toolKey === 'dashboard' || item.to === '/app'
    }
    if (!item.toolKey || item.toolKey === 'settings' || item.toolKey === 'dashboard') {
      return true
    }
    const roleOk = isPrivileged || accessibleTools.includes(item.toolKey as any)
    if (!roleOk) return false
    const mod = ROUTE_MODULE[item.to]
    if (mod && !isPrivileged && !accessibleModules.has(mod)) return false
    if (selectedTools && !selectedTools.has(item.toolKey as ToolKey)) return false
    return true
  }
    ...
    <div className="min-h-screen bg-[var(--av-surface-2)]">
      <aside className="hidden md:flex w-60 shrink-0 bg-[var(--av-surface)] border-r border-[var(--av-border)] flex-col fixed inset-y-0 left-0 z-30">
        ...
        <div className="px-3 pt-3">
          <button onClick={() => navigate('/app/capture')} className="w-full flex items-center justify-center gap-2 rounded-[var(--av-radius-md)] bg-[var(--av-primary)] text-white text-sm font-medium py-2.5 hover:bg-[var(--av-primary-hover)] transition shadow-[var(--av-shadow-sm)]">
            <Sparkles size={16} />
            Quick Capture
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-0.5">
          {NAV_GROUPS.map((group) => {
            ...
            return (
              <div key={group.id}>
                <button onClick={() => setOpenGroups(s => ({ ...s, [group.id]: !open }))} className={`w-full flex items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-semibold uppercase tracking-wide transition ${active ? 'text-[var(--av-text)]' : 'text-[var(--av-text-muted)] hover:text-[var(--av-text-secondary)]'}`}>
                  <GIcon size={15} strokeWidth={2.2} />
                  <span className="flex-1 text-left">{group.label}</span>
                  <ChevronDown size={14} className={`transition-transform ${open ? '' : '-rotate-90'}`} />
                </button>
                {open && <div className="ml-1 mt-0.5 space-y-0.5">
                  {visibleItems.map((item) => {
                    const Icon = item.icon
                    return <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `flex items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-sm transition ${isActive ? 'bg-[var(--av-primary-soft)] text-[var(--av-primary)] font-medium' : 'text-[var(--av-text-secondary)] hover:bg-[var(--av-surface-2)] hover:text-[var(--av-text)]'}`}>
          ...
          <div className="pt-2 mt-2 border-t border-[var(--av-border)]">
            {SECONDARY_LINKS.filter(itemVisible).map((item) => {
              const Icon = item.icon
              return <NavLink key={item.to} to={item.to} className={({ isActive }) => `flex items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-sm transition ${isActive ? 'bg-[var(--av-primary-soft)] text-[var(--av-primary)] font-medium' : 'text-[var(--av-text-muted)] hover:bg-[var(--av-surface-2)] hover:text-[var(--av-text)]'}`}>
                <Icon size={16} strokeWidth={2} />
                {item.label}
              </NavLink>
            })}
            <NavLink to="/app/more" className={({ isActive }) => `flex items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-sm transition ${isActive ? 'bg-[var(--av-primary-soft)] text-[var(--av-primary)] font-medium' : 'text-[var(--av-text-muted)] hover:bg-[var(--av-surface-2)] hover:text-[var(--av-text)]'}`}>
        ...
        <div className="px-3 pb-2">
          <a href="/upgrade" className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-gradient-to-r from-[#4285F4] to-[#8B5CF6] text-white text-sm font-medium hover:shadow-[var(--av-shadow-md)] transition">
            <Crown size={16} />
        ...
        <div className="relative border-t border-[var(--av-border)]">
          <button onClick={() => setUserMenuOpen(o => !o)} className="w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-[var(--av-surface-2)] transition">
            <div className="w-8 h-8 rounded-full bg-[var(--av-primary)] text-white flex items-center justify-center text-sm font-semibold shrink-0">{(staff?.full_name ?? staff?.name ?? '?').charAt(0).toUpperCase()}</div>
          ...
          {userMenuOpen && <div className="absolute bottom-full left-0 right-0 mb-1 mx-3 rounded-xl border border-[var(--av-border)] bg-[var(--av-surface)] shadow-[var(--av-shadow-lg)] overflow-hidden">
            ...
            <div className="border-t border-[var(--av-border)]"><button onClick={() => { setUserMenuOpen(false); signOut() }} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-[var(--av-danger)] hover:bg-[var(--av-danger-soft)] transition"><LogOut size={16} />Sign out</button></div>
          </div>}
        </div>
      </aside>
      ...
      <header className="hidden md:flex items-center gap-3 fixed top-0 right-0 left-60 h-14 px-6 bg-[var(--av-surface)] border-b border-[var(--av-border)] z-20">
        <button onClick={handleSearch} className="flex items-center gap-2 flex-1 max-w-md rounded-lg border border-[var(--av-border)] bg-[var(--av-surface-2)] px-3 py-1.5 text-sm text-[var(--av-text-muted)] hover:border-[var(--av-border-strong)] hover:bg-[var(--av-surface)] transition">
          <Search size={16} />
      ...
      <header className="md:hidden flex items-center justify-between px-4 py-3 border-b border-[var(--av-border)] sticky top-0 z-10 bg-[var(--av-surface)]">
        <div className="flex items-center gap-2">
          ...
          {branding.logo_url && <img src={branding.logo_url} alt="Logo" className="h-5 w-auto object-contain" />}
          <span className="text-sm font-semibold tracking-tight truncate text-[var(--av-text)]">{companyName}</span>
        </div>
        <div className="flex items-center gap-2">
          <NotificationBell />
          <button onClick={() => navigate('/app/capture')} className="w-8 h-8 rounded-full flex items-center justify-center bg-[var(--av-primary)] text-white"><Sparkles size={15} strokeWidth={2} /></button>
        </div>
      </header>

      <main className="md:ml-60 md:mt-14 p-4 md:p-6 pb-28 md:pb-8">
        {dbState === 'migrations-missing' && <div className="rounded-xl bg-amber-50 border border-amber-300 p-4 text-sm text-amber-800 mb-4"><strong>Database setup incomplete.</strong> Some features may not work because database migrations haven't been applied yet. An administrator needs to run the migrations in the Supabase Dashboard (SQL Editor → paste <code className="bg-amber-100 px-1 rounded">APPLY_ALL_MIGRATIONS.sql</code> → Run).</div>}
        <Outlet />
      </main>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 border-t border-[var(--av-border)] px-2 py-2 z-20 bg-[var(--av-surface)]">
        <div className="flex items-center justify-around">
          {MOBILE_NAV_ITEMS.map((item) => {
            const Icon = item.icon
            return <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg transition ${isActive ? 'text-[var(--av-primary)]' : 'text-[var(--av-text-muted)]'}`}><Icon size={20} strokeWidth={2} /><span className="text-[10px] font-medium">{item.label}</span></NavLink>
          })}
        </div>
      </nav>
    </div>
