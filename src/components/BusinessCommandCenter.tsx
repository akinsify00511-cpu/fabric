...
function contextSummary(context: Context, brain: BusinessBrain | null, health: BusinessHealth | null, ledger: ValueLedger | null) {
  if (context === 'Cash') return ledger ? 'Verified value and cash signals' : 'Cash picture is still forming'
  if (context === 'Business') return health?.overall_score != null ? `Business health score: ${Math.round(health.overall_score)}` : 'Whole-business operating view'
  if (context === 'Revenue') return brain ? 'Revenue and pipeline decisions' : 'Revenue picture is still forming'
...
export default function BusinessCommandCenter({ brain, health, recommendations, ledger, actions, functionLabel, seniorityLabel }: Props) {
  ...
  const objectiveText = brain ? 'Avenize is connecting your verified business signals to the next decision.' : 'Avenize is assembling the clearest next decision from your business data.'
  return (
      ...
      <div className="rounded-[28px] border border-[var(--av-glass-border)] bg-[var(--av-glass-bg-strong)] overflow-hidden shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
        <div className="px-5 sm:px-7 pt-6 pb-5 border-b border-[var(--av-glass-border)]">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
            <div>
              <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.12em]" style={{ color: 'var(--av-text-muted)' }}><Compass size={14} aria-hidden="true" />Business command center</div>
        ...
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] divide-y lg:divide-y-0 lg:divide-x" style={{ borderColor: 'var(--av-glass-border)' }}>
          ...
          <div className="p-5 sm:p-7">
            ...
            <div className="space-y-0">{[
              ...
              ['Next move', actions[0]?.title || recommendations[0]?.statement || 'Review the business picture'],
