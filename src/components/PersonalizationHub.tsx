...
export default function PersonalizationHub() {
  ...
  return (
    <div className="fixed bottom-24 right-4 z-40 w-80 bg-white rounded-2xl shadow-2xl border border-black/[0.06] overflow-hidden animate-in slide-in-from-bottom">
      {/* Header */}
      <div className="bg-gradient-to-r from-[var(--avenize-primary)] to-purple-500 p-4 text-white">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-2 rounded-lg bg-[var(--avenize-primary)] text-white text-sm font-medium"
          >
            Try it
          </button>
          <button
            ...
            {insights.suggestions.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition ${
                  i === activeSuggestion ? 'bg-[var(--avenize-primary)]' : 'bg-black/20'
                }`}
              />
            ))}
          </div>
...
export function UserInsightsPanel() {
  ...
  return (
    <div className="space-y-6">
      {/* Engagement Score */}
      <div className="bg-gradient-to-br from-[var(--avenize-primary)] to-purple-600 rounded-2xl p-6 text-white">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full mx-4 text-center animate-in zoom-in">
        ...
        <button
          onClick={onDismiss}
          className="w-full py-3 rounded-xl bg-[var(--avenize-primary)] text-white font-semibold"
        >
          Awesome!
        </button>
      </div>
