...
import { Check, ChevronDown, CircleDot, Grid2X2, LineChart, PieChart, Table2, TrendingUp } from 'lucide-react'
...
// One request is shared by every widget mounted on a dashboard. Local storage
// remains the instant fallback, while Supabase becomes the cross-device source
// of truth once the authenticated user is available.
  ...
  const promise = supabase
    ...
    .then(({ data }) => {
      ...
      return Object.fromEntries(
        Object.entries(raw).filter(([, value]) => VIEWS.some(view => view.id === value)),
      ) as Record<string, FabricView>
    })
    .catch(() => ({}))
...
async function savePreference(userId: string, widgetKey: string, view: FabricView) {
  try {
    ...
    // Preference persistence is intentionally best-effort. The view itself
    // must never fail because a preference write is unavailable.
  }
}
...
export default function FabricViewEngine({
  ...
  goalLabel = 'Goal', currency = '₦', description, defaultView = 'kpi', className = '',
  ...
  const [view, setView] = useState<FabricView>(() => {
    try {
      ...
      return saved && VIEWS.some(item => item.id === saved) ? saved : defaultView
    ...
    <section className={`rounded-3xl bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,.04),0_8px_24px_rgba(0,0,0,.04)] ${className}`}>
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="relative shrink-0">
          <button type="button" aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen(v => !v)} className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 transition hover:border-slate-300 hover:bg-slate-50">
            <SelectedIcon size={13} /> View <ChevronDown size={13} />
          </button>
          {open && <div className="absolute right-0 z-30 mt-2 w-44 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-xl">
            <div className="px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Represent as</div>
            {VIEWS.map(item => { const Icon = item.icon; const active = item.id === view; return (
              <button key={item.id} type="button" onClick={() => selectView(item.id)} className={`flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-xs transition ${active ? 'bg-slate-100 font-semibold text-slate-900' : 'text-slate-600 hover:bg-slate-50'}`}>
                <Icon size={14} /><span className="flex-1">{item.label}</span>{active && <Check size={13} />}
              </button>
            )})}
          </div>}
        </div>
