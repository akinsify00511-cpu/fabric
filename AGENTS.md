...
Set<string> in schemaAvailability resets on eachevery page reload, so the
FIRST probe per reload re-fires the doomed request, re-flooding the console
on every refresh.

### Fix
- `schemaAvailability` now persists verdicts to `sessionStorage` with a
  5-minute TTL (module state still takes immediate priority). Reloads within
  the TTL skip the first probe entirely; TTL expiry allows a just-applied
  migration to be picked up. All storage access wrapped — private-mode/SSR
  failures never break the guard.
- Additional Cockpit null-safe sites (the user re-hit their crash):
  `DiagnosisItem.recall.matches`, `BusinessHealthCard.dimension_scores.metrics`,
  `EffectivenessCard.rows` — all `?? []` hardened.
- Verified served bundle (`index-Cd9n4bVJ.js`).

### Why this matters
`avenize.riverwayse.com` has a service worker with `networkFirstWithOfflineFallback`
on navigations. Feature-first pattern means a stale cached shell hits the user
until the SW updates. With sessionStorage the verdict survives until SW swap;
without it, every reload re-fires the doomed probe.

Verified: tsc 0, build 0 warnings, vitest 655/655, schema-drift 0,
design-constitution PASS. Commit 184693b (pushed; CI ✓; Deploy ✓).
...
1. `compute_ebitda`, `recommendation_effectiveness`,
   ...
   `get_alert_actions` were still calling `supabase.rpc` UNGUARDED ŌĆö every
...
2. TypeError crash `Cannot read properties of undefined (reading 'length')`
   in ExecutiveCockpit cards ŌĆö five card components iterated RPC-payload
...
- tsc clean, vite build 0 warnings, vitest 655/655, schema-drift 0,
  design-constitution PASS. CI Ō£ō, Vercel **Deploy Production Ō£ō 1m48s**,
  commit `00a129b` (pushed).

Lesson: guarding the RPC wrapper is not enough ŌĆö always defensively
...
## Session 42 (2026-08-21): Console-drift wall silenced ŌĆö usage_events + user_workspace_selections guarded
...
(deployment drift ŌĆö same class as Session 38b, but these three call sites
...
### Fix (commit cf00b75, pushed; deployed cf00b75 ŌåÆ production)
...
Verified: tsc clean, build 0 warnings, vitest 655/655, schema-drift 0,
design-constitution PASS. CI SUCCESS; Vercel Deploy Production Ō£ō 1m55s.

Lesson: any NEW `.from(<drifting_table>)` call site must use the
schemaAvailability guard the first time it is introduced ŌĆö not after the
...
### Cmd/Ctrl+K ŌĆö fixed (`4b17caf`, pushed)
Root cause: `useCommandPalette` registered `{key:'k', meta:true}` ŌĆö
...
"Search or jump toŌĆ”" button dispatches a synthetic event that the palette
listener now matches, so the click path also works. Tests:
`tests/frontend/lib/keyboardShortcuts.test.ts` (5) ŌĆö Ctrl+K, Cmd+K, bare-K
denial, strict meta fallback, shift/alt. Lesson: never bind navigation
shortcuts to meta-only.

### Task creation ŌĆö fixed (`4b17caf`, pushed)
`Tasks.tsx` gated ALL creation behind `canManage`
(owner/admin/manager/team_lead) ŌĆö a staff member had no "Assign Task"
...
Verified: tsc clean, build 0 warnings, vitest 655/655, schema-drift 0,
design-constitution PASS. CI SUCCESS; Vercel Deploy Production Ō£ō 1m34s.
...
- Meetings detail "Join in Avenize" ŌåÆ /app/meeting-capture?meeting=<id>;
  legacy meeting_link can't take users outside.
- captureAttachments.ts uses the schemaAvailability circuit breaker.

### auth_rate_limits RLS advisor finding ŌĆö CLOSED (migration zzzb_security_tables_lockdown.sql)
...
SELECT/INSERT/UPDATE/DELETE ON ALL TABLES to authenticated ŌĆö so the table is
...
reproduce RLS-disabled + blanket-grant state ŌåÆ apply twice (idempotent) ŌåÆ
relrowsecurity=t ŌåÆ SET ROLE authenticated direct SELECT = permission denied
ŌåÆ check_auth_rate_limit returns. Lessons: (1) docker exec needs -i or stdin
is detached and psql -f silently runs nothing; (2) assert denial with raw
output ŌĆö grep for "does not exist" misses "permission denied".
...
- Premium.tsx design-constitution drift growth (hex 0->2) ŌåÆ bg tokens. Gate
  now PASS (125 files vs baseline 127).

### CI failures resolved
The merged HEAD (9402101) had red CI: design-constitution (Premium hex) +
migration-test FAIL on 20260820210000_native_meetings ŌåÆ Vercel deploys being
...
staff_id + text/voice/image/file/file/recording shapes ŌĆö CREATE TABLE IF NOT
EXISTS skipped ŌåÆ policies referencing staff_id aborted the file ŌåÆ
...
## Session 38b (2026-08-20): Schema-drift circuit breaker ŌĆö console wall silenced client-side
...
path preserved). Reload re-probes ŌĆö a just-applied migration is picked up
immediately. Guard signatures use PromiseLike (supabase builders are
thenables ŌĆö caught by tsc). Wired into businessOS RPC wrappers (business_brain,
current_metrics, open_recommendations, business_value_ledger,
profitability_leakage), BusinessHome loaders (email_campaigns,
attendance_records, staff:active-filter ŌĆö keyed per-filter since the missing
...
baseline ŌĆö the migration chain has still not been applied (all the same
...
ever `CREATE TABLE IF NOT EXISTS` (002/032/039) ŌĆö a no-op against the
...
path ŌĆö CREATE TABLE IF NOT EXISTS silently preserves the wrong shape.

**Tested on postgres:15 (Docker):** (A) simulated divergent live shape
(is_active staff, slug-less businesses, narrow legacy status CHECK) ŌĆö all
assertions pass, app write path (pendingŌåÆapprovedŌåÆcancelled) works, legacy
...
orphaned-membership reconciliation for user 361710ac-ŌĆ” / business f2d580d1-ŌĆ”.
## Session 39 (2026-08-20): Demand Capture ŌĆö Lead ŌåÆ Request ŌåÆ Quote ŌåÆ Order (commit a1e9b19, local, NOT pushed)
...
Architecture rule followed: NOT three disconnected modules ŌĆö one chain where
...
- quotes needed: client_name/client_email/quote_number are NOT NULL (048) ŌĆö
...
- notifications needs category enum (036) + type text (040); the demand_notify
  trigger supplies both, wrapped best-effort (EXCEPTION ŌåÆ NULL) so a
  notification failure never breaks a demand write.
- Trigger gotcha: CASE expressions resolve NEW.<field> for ALL branches ŌĆö use
...
- New tables need explicit GRANT SELECT/INSERT/UPDATE/DELETE TO authenticated
  ...
  GRANT USAGE on the identity sequences ŌĆö otherwise PostgREST reads 401.
...
- `supabase/migrations/zzzaaa_demand_capture.sql` (applies after zzz_auth):
  ...
  custom; lifecycle newŌåÆreviewingŌåÆqualifiedŌåÆquotedŌåÆacceptedŌåÆfulfilled with
  rejected/abandoned that can be REVIVED ŌĆö no lost demand), sales_orders
  (confirmedŌåÆin_fulfilmentŌåÆfulfilledŌåÆcompleted/cancelled), demand_activity
...
- 4 member-guarded SECURITY DEFINER RPCs: create_lead_request (auto-advances
  lead newŌåÆcontacted), create_quote (item-sum totals, auto requestŌåÆquoted),
  create_sales_order (REQUIRES accepted quote; backfills the whole chain;
  converted quotes can't double-order; requestŌåÆaccepted + quoteŌåÆconverted),
  transition_demand (validated per-entity lifecycles + timestamps).
- Public portal RPCs: get_quote_by_token (view, auto sentŌåÆviewed),
  respond_to_quote (accept/reject, once only) ŌĆö granted to anon.
- demand_notify triggers ŌåÆ existing notification bell (assignee, else owners).
...
- Frontend: src/lib/demand.ts; DemandActionCentre on the lead page (the
  "action centre" ŌĆö request/quote/order forms + visible chain + transitions +
...
- WhatsApp sweep: Pricing CTA, Signup headline + feature line, invoice
  empty-state tips ŌåÆ native demand-capture language. Landing/index already
...
leadŌåÆrequestŌåÆquoteŌåÆportal-accept(anon)ŌåÆorderŌåÆfulfilledŌåÆcompleted; cross-
...
key (extracted from the deployed bundle ŌĆö the bundle IS the new auth build,
`onboarding_required` present, so frontend is current):

**Probed live state (definitive):** staff has `is_active` NOT `active`
(legacy hand-built schema ŌĆö repo 002 ADD COLUMNs `active` additively, safe);
...
was hand-managed ŌĆö never tracked via supabase_migrations.

**The 406:** quality-control.ts used `.single()` for staff attribution ŌåÆ
PGRST116 for staffless users. Fixed ŌåÆ .maybeSingle() (commit 73cc42d).

**Shipped (commit 73cc42d, pushed to main):**
- scripts/apply_migrations_live.sh ŌĆö psql loop over all migrations in
  filename order, two-pass, per-file report to
  supabase/migration_apply_report.txt. Needs SUPABASE_DB_URL (Dashboard ŌåÆ
  Settings ŌåÆ Database ŌåÆ connection string).
- scripts/verify_live_db.sh ŌĆö re-probes the exact objects the frontend needs
  with SUPABASE_ANON_KEY. Baseline today: all FAIL (expected pre-apply).
- LIVE_DB_APPLY_RUNBOOK.md ŌĆö probed findings table, apply steps, pg_cron
  note, and an orphaned-membership reconciliation snippet (user
  361710ac-ŌĆ” references business f2d580d1-ŌĆ” but has NO staff row ŌåÆ after
...
## Session 36 (2026-08-20): Canonical auth lifecycle repair ŌĆö commit 795c359 (local, NOT pushed)
...
1. `create_business_and_owner` was broken on ANY fully-migrated DB, in THREE
   ...
   migrated DB. CI never caught it ŌĆö the migration job smoke-tests SELECT
...
2. `zz_rpc_tenant_guards_closure` re-declared `log_security_event` with a
   ...
   business_id ŌåÆ every security event silently dropped. Pre-auth RPCs must not
   be membership-guarded.
3. 999's `check_auth_rate_limit` counted every CHECK as an attempt ŌĆö 5
...
4. PostgREST returns TABLE RPCs as ARRAYS; Login read `rl.allowed` off the
   array ŌåÆ `!undefined` = true ŌåÆ the moment the RPC deployed, EVERY login would
   show "Too many failed attempts". Centralized normalization in
   src/lib/authSecurity.ts (`normalizeRateLimitRows`).
5. The grant migration (20260818290000) ran BEFORE 999 created the functions ŌĆö
...
- AuthContext: `MembershipState` = loading | anonymous | member |
  ...
  errors ŌåÆ membership 'error' (retry-in-place, never logout). refreshStaff()
...
- RequireAuth/OnboardingGate route purely off membership; dead RequireSession
  ...
  Signup redirects already-authenticated visitors (memberŌåÆ/app,
  onboarding_requiredŌåÆ/onboarding). Onboarding/Join/AuthCallback use
  ...
  await refreshStaff() + navigate ŌĆö all window.location.href auth transitions
...
zzz_auth_protocol_repair.sql (+ the pending set) applied ŌĆö until then the
frontend fails open exactly as before (rate-limit unavailable ŌåÆ proceed;
business-creation RPC missing ŌåÆ honest "not configured" message).

## Session 34 (2026-08-19): Two formal product layers ŌĆö Design Intelligence + Discovery Intelligence
...
the architecture ŌĆö never plugins. 4 commits (28c237e..5d92e96), all local
on main (NOT pushed ŌĆö awaiting user confirmation per repo policy).

### Phase A ŌĆö Design Constitution + enforcement (28c237e)
- `AVENIZE-DESIGN-CONSTITUTION.md`: the governing document ŌĆö A1 foundations
  ...
  experience (points to functionHome.ts ŌĆö UX emphasis only, RLS stays the
...
### Phase B foundation ŌĆö technical SEO + the public/private boundary (59db5b2)
- `public/robots.txt`: B3 boundary ŌĆö public surface allowed; /app, /sign,
...
- `public/sitemap.xml`: public routes only. Generated real og-image.png
  (1200x630) + logo.png (512) ŌĆö both were referenced but 404'd.
- **┬¦22/B5 structured-data truth fixes in index.html**: removed fabricated
  aggregateRating (4.8/1247 ŌĆö a Google policy violation), fake "12 hours/week"
  + "SOC 2 Type II" FAQ claims, and USD $29/$49 schema pricing that
  contradicted the real Naira tiers (now NGN 0ŌĆō380,000). theme-color ŌåÆ #155BB4.
- Renumbered 20260819020000_receipt_ocr ŌåÆ 20260819025000 (collision with
  audit_logs_column_reconciliation; both unapplied to live DB, safe rename).

### Phase B intelligence ŌĆö the Discovery layer (59a02cc + 5d92e96 doc)
- Migration `20260819090000_discovery_intelligence.sql` (idempotent, verified
  ...
  discovery_content / discovery_referrals ŌĆö business-scoped RLS
...
- `src/lib/discoveryIntel.ts`: pure deterministic logic ŌĆö presence/citation
  ...
  "is a/an/the" compared separately ŌĆö name overlap never masks a category
  error; disjoint+brand-absent=critical, disjoint=high), B10 opportunity
  priority, B14 attribution rollup + UTM/referrer parsing (AI engines ŌåÆ
...
- `src/lib/attribution.ts`: UTM/referrer capture on Landing/Pricing/Signup;
  ...
  discovery ŌåÆ visit ŌåÆ signup ŌåÆ business ŌåÆ subscription revenue).
- `DiscoveryIntelligence.tsx` at /app/discovery (Reach group): B13 overview +
  ...
  B10/B11 opportunities with quality gate, B14 Discovery ŌåÆ Revenue.
  Executives + marketing function only (UX gate; RLS+RPCs are the boundary).
  Gamified honest empty states; pure --av-* tokens (zero hex ŌĆö passes the new
  design gate).
...
- psql autocommits: set_config(..., true) is transaction-local and vanishes
  before the next statement ŌĆö use false (session) in ad-hoc .sql files, or
  wrap in BEGIN/COMMIT like 04_rls_attack_suite.sql.
- The postgres superuser BYPASSES RLS ŌĆö cross-tenant checks must SET ROLE
...
## Session 32 (2026-08-19): Avenize-first architecture ŌĆö internal Receipt OCR, internal WebAuthn/Passkeys, Generative Copilot

User directive: "no external SaaS dependency" does NOT mean no libraries or
platform primitives. Do NOT block features on external providers ŌĆö build
...
### Track 1 ŌĆö Internal Receipt OCR (commit 0483f6f, migration 20260819020000)
...
- `receipt_documents` (original + raw text + structured fields + per-field
  ...
  ({business_id}/ path convention, signed URLs only ŌĆö same pattern as
  ...
  cashflow_entries 004 expense + links, IDEMPOTENT ŌĆö repeat confirm returns
...
- Receipts.tsx (/app/receipts): drag-drop upload ŌåÆ in-browser OCR ŌåÆ editable
  review panel with confidence chips + low-confidence warning ŌåÆ Confirm
...
### Track 2 ŌĆö Internal WebAuthn/Passkeys (commit e5981b7ŌåÆ2decfa5, migration 20260819030000)
- Browser WebAuthn API ŌåÆ `webauthn` edge fn (server-side cryptographic
  verification via @simplewebauthn/server@13 in our own Supabase Edge
  runtime) ŌåÆ Postgres credential registry. No Auth0/Clerk/Okta.
- `webauthn_credentials` (public key + counter ONLY ŌĆö asymmetric by design,
  nothing secret stored; own-rows RLS; soft-revoke preserves audit),
  `webauthn_challenges` (single-use, 5-min TTL, client-DENIED ŌĆö service role
...
### Track 3 ŌĆö Generative Copilot (commit 48b72baŌåÆ76116be, migration 20260819040000)
...
- `src/lib/copilotRouter.ts` is the canonical, unit-tested router (the edge
  fn mirrors it ŌĆö Deno can't import src/; keep in sync). Contract (┬¦22):
  answers quote ONLY assembled-context values; missing data ŌåÆ honest no-data
  answer + the action that creates it.
- `copilot_messages` (business-scoped RLS; edge-fn-only writes) +
  copilot_daily_usage RPC (membership-guarded ŌĆö the #18 lesson applied).
- AskAvenize.tsx (/app/ask): chat UI, provider badge ("Answered from your
  ...
  Ask Avenize buttons now land here; Quick Capture still ŌåÆ /app/capture.
...
- **Full-chain migration apply is now 163/163 GREEN** on bare postgres:15 +
  ci_shim ŌĆö the remote commit c1a4e24 ("fix(ci): repair all broken
...
tsc clean; vite build 0 warnings; vitest 516 ŌåÆ 538 (+22) ŌåÆ 551 (+13) ŌåÆ
566/566 (+15); schema-drift 0; all 3 migrations apply clean + idempotent
against postgres:15; full-chain apply 163/163.

### Deploy status
- ŌÜĀ’ĖÅ STILL needs live DB: migrations 20260819020000ŌĆō20260819040000 (+ the
...
## Session 31 (2026-08-19): Verified-audit triage ŌĆö account member kinds (the one genuine P0 gap)
...
### Verified STALE audit claims (already built ŌĆö do NOT rebuild)
- **Autonomous trial feature discovery:** audit says missing. REALITY: built
  Session 23 ŌĆö `feature_discovery` RPC (migration 20260818190000,
...
- **AI plan recommendation at trial end:** built Session 23 ŌĆö
...
- **Role-specific home experiences:** audit says partial (owner/manager/staff
  only). REALITY: Function ├Ś Seniority homes built Session 29 ŌĆö
...
- **AICapture:** confirmed real (parse-intent edge fn + emit_business_event +
  destination propagation) ŌĆö not frontend-only. Audit itself agreed.

### GENUINE gap closed this session ŌĆö account-type architecture (commit 762a5c0)
...
- **Migration 20260819015000_account_member_kinds.sql** (idempotent, verified
  ...
  - `staff.member_kind TEXT NOT NULL DEFAULT 'staff'` + CHECK (owner/staff/
    consultant/vendor/expert/partner) + backfill (role='owner' ŌåÆ 'owner') +
    (business_id, member_kind) index.
  - `invites.member_kind` + CHECK. `create_invite` re-declared with
    p_member_kind (validates invitable kinds; 'owner' NEVER invitable ŌĆö
  ...
  - `accept_invite` re-declared: carries member_kind into the staff row.
    **Pre-existing defect found + fixed by the apply-test:** the 110 version
    inserted staff WITHOUT email while staff.email is NOT NULL ŌåÆ invite
...
- **SECURITY invariant (locked in tests):** member_kind is IDENTITY/UX
  composition only ŌĆö `staff.role` + RLS stay the authoritative boundary
...
### Verification
tsc clean (after `npm install` ŌĆö the container had no node_modules; npx was
...
### Deploy status
Committed locally (762a5c0, 5 files, +536/-10). NOT pushed ŌĆö awaiting user
...
## Session 33b (2026-08-19): Red-team authorization closure audit ŌĆö 3 P0 privilege-escalation vectors found + fixed
...
1. **Employee self-promotion to owner** ŌĆö `staff_self_update` UPDATE policy
   ...
   trigger ŌĆö role/member_kind mutations require the CALLER's own staff role
   to be owner/admin in the same business. Employee ŌåÆ ERROR; owner/admin ŌåÆ OK.

2. **Brain-poisoning via claims table** ŌĆö `claims_managing` policy gave every
...
3. **Organization-move via businesses update** ŌĆö `businesses_own_update`
   ...
   **FIXED** (20260819080000): `enforce_business_structural_immutability()`
   trigger ŌĆö structural fields require owner/admin.
...
## Session 33 (2026-08-19): Red-team audit closure ŌĆö RPC tenant guards + AI hardening
...
- Inventory (scripts/security_inventory.py): 417 functions scanned; 77
  ...
  guard ŌĆö callable cross-tenant because migration 998 blanket-GRANTed
...
- Verified context isolation: the edge fn assembles data with the caller's
  own JWT (user-scoped client) ŌĆö never a service-role aggregate.
...
## Session 26 (2026-08-18): Meeting, Communication & Meeting Intelligence ŌĆö Phases B-E (bounded subsystem)

User directive: build "Avenize Meeting, Communication & Meeting Intelligence"
ŌĆö audit existing architecture, reuse canonical systems, build as bounded
...
of the Phase A lifecycle (Session 24). Each phase: backend ŌåÆ frontend ŌåÆ
connect ŌåÆ test (tsc + build + vitest + schema-drift + postgres:15 migration
idempotency) ŌåÆ commit ŌåÆ push to main. 4 commits (f22a243..6e68722), all
pushed to main. tsc clean, vite build 0 warnings, vitest 447ŌåÆ504 (+57 new),
schema-drift 0, every migration applies clean + idempotent.

### Phase B ŌĆö Recording + Capture (commit f22a243, migration 20260818500000)
The recording layer + Loom-style async captures. Reuses meeting_media (Phase
A) + the meeting-recordings bucket ŌĆö NOT a parallel media system.
- **CRITICAL security fix (┬¦32):** Meetings.tsx used `getPublicUrl()` on the
  PRIVATE meeting-recordings bucket ŌåÆ returned 404 + exposed the path. The
  new flow: `create_recording_upload_path` RPC (creates a pending media row +
  returns a private storage path) ŌåÆ upload ŌåÆ `finalize_recording` RPC (marks
...
- **meeting_captures table:** Loom-style async recordings (screen/camera/
  screen_with_camera/audio_only) ŌĆö not tied to a live meeting. RLS business-
...
- 7 SECURITY DEFINER RPCs (all membership-guarded): create_recording_upload
  ...
  _recording_signed_url (AUTH GATE ŌĆö verifies membership before returning
...
- Meetings.tsx `saveRecording` replaced: getPublicUrl ŌåÆ signed-URL flow.
...
### Phase C ŌĆö Transcript + Summary + Decisions + Actions (commit f9d2bf1, migration 20260818600000)
The intelligence layer ŌĆö recordings become structured, searchable, actionable.
- 5 relational tables: meeting_transcripts (full_text + word_count + duration
  ...
  superseded ŌĆö reversed stays VISIBLE for audit trail, links to claims for
  the ┬¦15 outcome loop), meeting_actions (text + assignee + due_date + 5-status
  lifecycle, links to REAL tasks table 004 via task_id ŌĆö NOT a parallel task
...
- **transcribe-audio edge fn extended:** GPT-4 now extracts STRUCTURED
  decisions + actions (best-effort, non-fatal ŌĆö transcript + summary are the
...
- **MeetingIntelligenceView.tsx** page (/app/meetings/:meetingId/intelligence):
  ...
  due date + status + actionŌåÆtask creation modal), expandable transcript,
  transcript full-text search.
- 16 tests (meetingTranscriptDecisions): transcript model, decision lifecycle
  (4 statuses, reversed=visible audit trail), actionŌåÆtask linking (NOT a
  parallel task system), 5-status action lifecycle, 4-priority levels, meeting
  intelligence aggregation, cross-tenant denial, actionŌåÆtask linking.
- Also fixed pre-existing CRM.tsx prop mismatch (CRMIntelligenceSurface takes
  `compact`, not `businessId` ŌĆö was a latent TS error).

### Phase D ŌĆö Post-Meeting Report + Notifications (commit f11f731, migration 20260818700000)
...
- **index.css:** print styles (@media print ŌĆö .no-print hidden, white bg).
...
### Phase E ŌĆö Analytics + Productivity Intelligence (commit 6e68722, migration 20260818800000)
The cross-meeting productivity view. Read-only analytics over existing data
(no new tables ŌĆö pure interpretation).
- **meeting_analytics RPC:** composes productivity metrics across ALL meetings
  ...
  transcript adoption), action completion % (NULL when no actions ŌĆö honest,
  not 0%), wasted-meetings detection (meetings with no decisions AND no
  actions ŌĆö the time-waste signal), per-staff meeting load (created +
...
- **Phase A (Session 24):** the lifecycle ŌĆö create_meeting, start_meeting,
  join_meeting (token-gated), leave_meeting, end_meeting, + meeting_participants
  + meeting_participant_events (evidence).
- **Phase B:** recording + capture ŌĆö the meeting or an async capture is
...
The phases compose into a single coherent Meeting Intelligence subsystem ŌĆö
...
### Verification (every commit + final)
tsc clean; vite build 0 warnings; vitest 447ŌåÆ504 (+57: meetingRecordingCapture
...
(verified via local Docker pg-test container ŌĆö the CI bare-postgres migration-
test job equivalent).

### Deploy status
- Vercel production: all commits deploying via the main-push workflow.
- ŌÜĀ’ĖÅ STILL needs live DB: migrations 20260818500000ŌĆō20260818800000 (Phases B-E)
...
## Session 25 (2026-08-18): Ready-pillars ŌĆö Memory Recall (┬¦I), Resilience (┬¦N), Multi-role switching (┬¦K)
...
### ┬¦I ŌĆö Business Memory Recall ("What happened before?") ŌĆö commit 5dfe294
...
- **Persistence gap fixed:** the Brain's `diagnose_business` was EPHEMERAL ŌĆö
  ...
  inside the existing per-rule EXCEPTION block (┬¦24 ŌĆö a persistence failure
  never breaks the diagnosis).
- **Evidence-tag contract (┬¦20/┬¦22 anti-fabrication core):** prior_diagnosis
  ŌåÆ FACT (it happened); decision w/ actual_outcome ŌåÆ FACT; decision w/o
  outcome ŌåÆ INFERENCE; organizational_memory ŌåÆ INFERENCE (a learned lesson
  is a generalization). Honest empty note when nothing matches ŌĆö NEVER
...
- **Surfaced on ExecutiveCockpit DiagnosisCard:** each diagnosis gets a
  ...
  tag ŌĆö the directive's "you encountered this 6 months ago; you tried X, result
...
### ┬¦N ŌĆö Platform Resilience ("too good to fail or break down") ŌĆö commit 51db179
...
- **Brain graceful degradation:** `business_brain` called all 5 sub-engines
  inline ŌĆö if ANY threw, the outer EXCEPTION blanked the ENTIRE response
...
- **Automation retry + dead-letter queue:** `automation_runs` (007) logged
  ...
  turn the existing table into a retry+DLQ state ŌĆö no new table (┬¦0.5).
...
### ┬¦K ŌĆö Multi-role switching ("Owner vs Staff vs Multiple Roles") ŌĆö commit 9981ec0
...
- **staff_secondary_roles** table ŌĆö secondary business roles. Effective
  permission level = MAX(primary, secondary) ŌĆö a secondary role can only ADD
  access the user is entitled to, never remove (UNION). RLS: self-manage +
  owner/admin business-wide.
- **active_role session state** ŌĆö which persona the user is operating as now.
  ...
  hold. UX/context ONLY ŌĆö RLS + staff.role remain the security boundary (matches
...
- **Client:** `RoleSwitcher` component in the Shell user card (only appears when
  the user holds >1 role ŌĆö the common single-role case sees nothing). Shows each
...
### Still pending (blocked on live DB ŌĆö same gate as prior sessions)
...
### ┬¦G/┬¦J/┬¦AA ŌĆö three more composition-first ready-pillars ŌĆö commit c1d13b4
Continued with the next three named pillars. Each audited existing infra first
and builds on the established spine ŌĆö no parallel systems.
...
- **┬¦G Profitability Intelligence** ("Where is the business making/losing
  ...
  salesperson / channel ŌĆö cost is revenue-proportionally allocated, surfaced
  ...
  declining-margin customers, underpriced won deals, stale receivables ŌĆö each
...
- **┬¦J Business Graph impact propagation** ("if this closes, what else
  ...
  revenue/cash effect per edge ŌĆö FACT for invoice/payment/deal, INFERENCE for
...
## Session 23 (2026-08-18): Trial-experience engine ŌĆö P0 #1, #13, #14, #15, #16 (deterministic, zero LLM)
...
usage/onboarding data ŌĆö no LLM, no fabricated numbers, no fabricated urgency.
...
the GitHub repo slug "akinsify00511-cpu/fabric" (the actual repo name ŌĆö not
...
Inventory" and explains why it matters ("could help you identify Ōé”X in
...
- Migration 20260818190000: `module_value_propositions` table (the "why this
  ...
  estimate). Tunable by Avenize operators (service role) ŌĆö Riverwayse decides
...
- `feature_discovery(business_id)` RPC: returns modules the business is
  ...
  via format(%L) ŌĆö never string concat from client input) + explore route.
  Ordered: non-zero estimates first (desc by value ŌĆö highest-impact unexplored
...
- Client: Dashboard "Worth exploring" card (owner/admin only) + formatNaira.
  The estimate line is hidden when 0/null (never shows "Ōé”0" ŌĆö the ┬¦22 anti-
...
- PROBLEM FIXED: prices were hardcoded in TWO places (edge fn PLAN_PRICES +
  Pricing.tsx PLANS) ŌĆö a ┬¦0.5 single-source-of-truth violation. And
  business_entitlements.plan CHECK only allowed 4 codes while the others used
  5 (three sources, near-drift).
- Migration 20260818200000: `pricing_tiers` table ŌĆö the SINGLE source of
...
- `price_locked` flag on business_subscriptions: founding subscribers keep
  ...
  not the current tier price ŌĆö the price-lock guarantee).
...
- A price change / founding-period end / future-increase activation is now an
  UPDATE to pricing_tiers ŌĆö NO code change, NO redeploy.
...
- Migration 20260818180000: `recommend_plan(business_id)` RPC. SECURITY
  ...
  ŌĆö never fabricated, ┬¦22). reasons: why the plan fits. additional_value_unlocks:
...
- BUG FOUND + FIXED while writing tests: resolve_plan_tier (20260101000005)
  lacked 'business' and 'team' plan names ŌĆö a Business-plan user got tier 0,
...
- Phase taxonomy (priority-ordered, deterministic): setup_incomplete (highest)
  ...
  > healthy (no nudge ŌĆö don't nag). Not in trial -> no nudge.
- Client: fetchTrialAssistance wrapper. TrialBanner now ADAPTIVE ŌĆö shows the
...
The three compose into a single, coherent, deterministic trial experience ŌĆö
...
- `src/lib/AuthContext.tsx` `signOut` now calls `clearMfaVerified(userId)` so the per-session flag does not outlive the session. (Made the `./mfa` import static ŌĆö the dynamic-import build warning is gone.)
...
### S3 ŌĆö Stop loading provider secrets to the client
- **PaymentSettings.tsx:** `select('*')` ŌåÆ `select('id, provider, is_active, is_test_mode, status, supported_currencies')`. The `secret_key_encrypted`/`webhook_secret_encrypted` columns are no longer sent to the browser. (Plaintext-at-rest insert issue remains a server-side follow-up ŌĆö needs a pgcrypto encrypt RPC; flagged, not in this batch.)
- **smsService.ts:** Rewrote `TermiiSMS.getBalance` to route through the `send-sms` edge function (`action: 'balance'`) ŌĆö no client-side API key. Removed `getConfig` (returned the key value to the client) and `sendDirect`. Removed the dead `SMSConfig` interface and unused `TERMII_API_URL` constant. Rewrote `TermiiOTP` (`send`/`verify`/`resend`) to route through the edge function (`otp_send`/`otp_verify`/`otp_resend`).
- **send-sms/index.ts (edge function):** Extended with `balance`, `otp_send`, `otp_verify`, `otp_resend` actions; `SMSRequest` gained an `action` field. Reads the Termii key from the `settings` table server-side (service role) ŌĆö never from client env.
...
### R1 ŌĆö .env.example + .single() hygiene
...
- `.single()` ŌåÆ `.maybeSingle()` on the SMS settings SELECTs (avoids a 406/error when no row exists yet).

### Migration 079 ŌĆö settings.type column + secret RLS (DATA INTEGRITY + SECURITY)
...
- **Plaintext-at-rest for payment gateway secrets** (`payment_gateways.secret_key_encrypted` is actually plaintext despite the column name). Needs a server-side pgcrypto encrypt/decrypt RPC + Edge Function update. Internal SQL ŌĆö no external dependency.
...
- ŌØī AI-purple gradients, centered hero over dark mesh
- ŌØī Three equal feature cards
- ŌØī Generic glassmorphism on everything
- ŌØī Inter + slate-900 defaults
- ŌØī Fake product previews built from styled divs
- ŌØī AI copywriting clich├®s ("Elevate", "Seamless", "Game-changer")

**Preferred patterns:**
- Ō£ģ Phosphor or Lucide icons (standardized stroke)
- Ō£ģ Motion animations for interactive elements
- Ō£ģ Skeleton loaders instead of spinners
- Ō£ģ Realistic content, not "John Doe" placeholders
- Ō£ģ Google Sans Flex typography
- Ō£ģ Token-based colors from brand system

---

## Brand System (Google Standard Edition)

Treat Avenize as if it were built by Google's Workspace team ŌĆö indistinguishable from Gmail, Calendar, or Admin Console.
...
1. **Every color from tokens** ŌĆö never hardcoded hex in components
2. **Cards: shadow, no border** ŌĆö use elevation tokens, never add borders to cards
3. **Typography: var(--font-family)** ŌĆö Google Sans Flex
4. **Radius from tokens** ŌĆö 8/12/16/24px only
5. **Spacing: 4px grid** ŌĆö --space-1 to --space-9
6. **Motion: 100-300ms** ŌĆö cubic-bezier(0.2, 0, 0, 1), no bounce/spring
7. **Gradient only marketing** ŌĆö never in /app/* chrome
...
- `isDemo` guards removed from pages and `TrialBanner` (AuthContext still exports `isDemo` but it's hardcoded `false` ŌĆö dead code, safe to ignore)
- `SarahChat` rebranded as "Help Guide" ŌĆö no more fake "AI Assistant" claims; `Bot` icon ŌåÆ `HelpCircle`
...
- `paystack.ts` `verifyPayment()` security bug fixed (was returning `true` ŌåÆ auto-approve; now returns `false`)
...
- `src/lib/DemoData.ts` still exports `DEMO_USER`/`DEMO_DEALS`/`clearDemoData`/`initDemoData` but is no longer imported anywhere ŌĆö candidate for deletion
- `DEMO_*` type-checking constants remain in some pages (unused in runtime paths)
- `AuthContext.tsx` still has `isDemo` (hardcoded `false`) ŌĆö dead code
...
- `Quotes.tsx` still uses `localStorage('avenize_quotes')` ŌĆö needs Supabase table migration
- Trial tracking is client-side `localStorage` ŌĆö should move to `business_entitlements.trial_ends_at`
...
- e-Signature engine (Sign) ŌĆö net new, no signing flow exists
- Property vertical ŌĆö net new (records, owners/tenants, sales, leasing, inspection)
- Website Builder ŌĆö net new
- Public Appointments ŌĆö net new
- Customer-facing Live Chat ŌĆö net new (existing chat is internal team chat)
- Vendor/PO workflow ŌĆö net new
...
- **Stack:** Vite + React 19 + TypeScript, Tailwind v4, Supabase (Postgres + RLS + Edge Functions), no dedicated backend server ŌĆö the SPA talks to Postgres directly via the Supabase SDK; RLS is the real authorization boundary.
- **Build/verify:** `npm install` ŌåÆ `npx tsc -b --noEmit` (typecheck) ŌåÆ `npx vite build` (production build). Both must pass before committing. No Postgres runtime in the dev container, so DB tests (`tests/database/*.sql`, pgTAP) and Playwright tests run in CI with a live Supabase.
- **RLS pattern:** business-scoped tables use `business_id = (SELECT business_id FROM public.get_current_staff())` where `get_current_staff()` (defined in `001_initial_schema.sql`) returns the current staff row via `auth.uid()`. Client-side `src/lib/permissions.ts` is UX gating only ŌĆö never the security control.
...
- **Two parallel stock models:** `products` (has its own `stock` integer column) and `inventory` (separate table, no `product_id` FK). `stock_movements.inventory_id` FKŌåÆ`inventory`. Update `products.stock` directly when receiving goods against a PO line that references `products`.
- **UIŌåöschema drift is the highest-risk defect class:** several pages (ElectronicSignatures, PublicAppointments) previously queried tables with no migration. When adding a page, confirm every `.from('table')` has a backing migration before shipping.
...
- `ElectronicSignatures.tsx`: removed demo-data fallback; now does a joined `signature_requests`Ōåö`signature_signers` query and a normalized two-step insert.
...
- **New management UIs** (all real-DB, RLS-scoped, token design system):
  - `PropertyOwners.tsx` ŌĆö CRUD owners with property-count join.
  - `PropertySales.tsx` ŌĆö offers/closings tracker; creates approval-gated commissions (inserts `property_commissions` + linked `approvals` row so payouts are manager-approved before Finance pays).
  - `Vendors.tsx` ŌĆö CRUD suppliers (contact, payment terms).
  - `PurchaseOrders.tsx` ŌĆö multi-line PO creation (auto-fills from product record), draftŌåÆsentŌåÆreceived lifecycle, goods-receipt recording that triggers `apply_goods_receipt` (auto-bumps `received_quantity`, advances PO status, increments `products.stock`).
- **Fake-data purge:** `OwnerInsights.tsx` was entirely hardcoded mock (`SARAH_STATS`, `MODULE_USAGE`, `RECENT_ACTIVITY`, `PERFORMANCE`) ŌĆö owner saw fabricated revenue/deal numbers. Rewrote to fetch real metrics (invoice revenue w/ MoM delta, deal/staff/property counts) and real activity from `audit_logs`. Sarah/Modules tabs now honestly state they need a dedicated analytics pipeline. `DocumentsHub.tsx` had the same demo-fallback anti-pattern masking the real `documents`/`document_folders` tables (038) ŌĆö removed.
...
- **3 missing RPCs the frontend called but no migration defined:** `update_leave_balance` (LeaveManagement.tsx ŌĆö approving leave never updated the balance), `increment_saved_search_use` (auditLogger.ts had a JS fallback), `increment_user_learning` (eventTracker.ts swallowed the error ŌĆö learning loop silently broken). All three defined in migration 071.
...
- **Verified:** `npx tsc -b` clean, `npx vite build` succeeds, `npx vitest run` 4 files / 61 tests passing. Files changed: `src/components/Shell.tsx` (rewritten, 438 lines), `src/pages/More.tsx` (rewritten, 206 lines), `src/App.tsx` (2 alias routes). No routes removed, no backend/RPC changes ŌĆö purely navigation IA + nomenclature + brand tokens.


## Session 7 (2026-08-09): close ALL docs-vs-codebase audit gaps ("okay lets fix all")

- **P0 modules built (missing from docs vs code):** Executive Cockpit (`/app/cockpit`) ŌĆö role-segregated CEO/CFO/COO decision view with drill-down + exception feed, distinct from the personal Dashboard; Company Wall (`/app/wall`) ŌĆö culture hub (recognition, announcements, events, birthdays, polls); Legal (`/app/legal`) ŌĆö contracts/cases/obligations with expiry+risk signals; Procurement RFQ (`/app/procurement`) ŌĆö requestŌåÆsolicitŌåÆcompareŌåÆPO loop (procurement previously stopped at PurchaseOrders+Vendors).
- **P1 intelligence:** Market Index (`/app/market`) surfaces `market_intelligence` RPC with evidence/confidence labels; Organizational Memory (`/app/memory`) ŌĆö institutional learning loop UI (searchable lessons feeding back into recommendations). IntelligenceHub now also calls `salary_affordability` + `compensation_review_recommendation` (16 RPCs ŌåÆ 11 wired) and every panel has an actionable drill-down link.
- **Evidence model platform-wide:** new `src/components/Evidence.tsx` exports `ClaimTag` (FACT/INFERENCE/ESTIMATE/RECOMMENDATION), `ClaimNote`, `ProvenanceLine` ŌĆö reusable so every business page labels what a number IS, per Master ┬¦20 / Thinking Framework ┬¦6.
- **Control-plane abstractions (┬¦2 "no module reinvents identity/permissions/audit/workflow"):** `src/components/ApprovalRouter.tsx` (`useApprovalRouting` hook + `ApprovalRequestButton`) wraps `start_approval_protocol` + `route_work` so any page requests/routes an approval in one line. `src/components/Reversal.tsx` (`useReversal` + `ReverseButton`) gives first-class undo/void/correct/amend with provenance (who/when/why/snapshot/related approval) into `action_reversals` ŌĆö originals not deleted, fully auditable (┬¦18, Law 8).
...
- **Schema (migration `20260101000004_gap_fill_modules.sql`):** legal_contracts/legal_cases/legal_obligations, purchase_requests/rfqs/rfq_line_items, polls/poll_votes, organizational_memory/decision_log, reality_gaps, action_reversals ŌĆö all with business-scoped RLS + updated_at triggers (follows the `staff.business_id IN (SELECT business_id FROM staff WHERE user_id = auth.uid())` pattern).
...
- **The gap:** the existing entitlement system (`business_entitlements` + `has_feature()` + `useEntitlement` + `EntitlementGate`) checked ONE dimension (entitlement) and was client-side only. A hidden nav item with an unprotected route behind it is not a gate ŌĆö a user could type the URL. There was no `module_ready` flag at all, so a paying customer could see a module running on fake/demo data.
...
- **Migration `20260101000005_two_flag_module_gate.sql`:** `module_plan_tiers` (module_key ŌåÆ min_plan_tier 0-3) + `module_status` (module_key ŌåÆ ready boolean). Plan tiers mapped to existing free/starter/professional/enterprise; roadmap growth/scale accepted as aliases (growthŌåÆprof, scaleŌåÆenterprise) so existing data is untouched. RLS: authenticated can READ config+status; only service role writes readiness.
- **Honest readiness seed (the safety net):** ready=true ONLY where the module persists real data today ŌĆö finance, chat, crm, tasks, hr, projects, inventory, knowledge, approvals, calendar, legal, procurement, intelligence, market, memory, reality_gap, self_audit, cockpit, wall. NOT ready (false): reports (mock aggregates), automations (no execution engine), sso (no IdP), api (no enforced key gating), multi_company (single-business only), security (audit enforcement incomplete). Flipping to true = the launch.
- **Route-layer enforcement (the P0 fix):** `RequireModule` component wraps every gated route via `mg(module, <El/>)` in App.tsx. A user typing `/app/automations` (not ready) hits the gate page, not the module. The gate distinguishes the two reasons: "not ready yet" (entitled but not ready ŌåÆ back-to-dashboard) vs "needs higher plan" (not entitled ŌåÆ upgrade CTA).
- **Sidebar filter:** `Shell.tsx` `itemVisible` now ANDs two gates ŌĆö (a) existing tool-role access, (b) new module gate via `useAccessibleModules` + a `ROUTE_MODULE` map (route path ŌåÆ ModuleKey). Settings/capture/dashboard always visible. A Starter customer now sees only Finance+Chat (the "easy to use" promise from day one).
...
- **Deploy note:** migration `20260101000005` must be applied to Supabase before the gate is effective. Before that, `can_access_module` errors ŌåÆ the gate treats it as "not ready" ŌåÆ most modules hidden (safe default). Apply the migration first.

## Session 9 (2026-08-10): Block A+ verification + applied intelligence + usage telemetry

Triggered by a rigorous roadmap review. The reviewer's core directive: *"force every ┬¦13.1 'Exists' module through the ┬¦9 production bar before building anything new"* and *"treat the generative AI Copilot as Phase 3 ŌĆö build Applied Intelligence (deterministic, Postgres-only) now instead."* Verified the reviewer's factual claims, then executed.

### Verified (the reviewer was right)
- `src/lib/features.ts` exists and says exactly what was reported: Ō£ģ production = 2FA + Nigeria Mode; ­¤¤Ī beta = webhooks + automations (need pg_net/pg_cron); ­¤ö┤ rest not started. Confirmed.
- AI Copilot = ­¤ö┤ "coming_soon, needs LLM integration" ŌĆö confirms zero of the intelligence layer exists. Confirmed.
...
- **Triple source of truth found (worse than the dual the reviewer named):** "can this user see X?" was answered by (1) `features.ts` hardcoded client object, (2) `module_status` DB table [Session 8 gate], (3) `PLAN_ENTITLEMENTS` hardcoded constant in `useToolAccess.ts` that **defaulted every non-privileged user to the Professional tool set regardless of their actual plan**. Source #3 was the dangerous one ŌĆö a free-tier user got pro tools.

### A+ work done (the must-be-first work)
1. **Reconciled source #3 ŌåÆ DB.** `useToolAccess` now loads the business's actual `business_entitlements.features` JSONB and derives the tool set via `derivePlanTools()` ŌĆö the SAME source `has_feature()` and `can_access_module()` read. The hardcoded `PLAN_ENTITLEMENTS` is gone. A free-tier user now sees only `BASE_TOOLS` (dashboard, crm, people, tasks, settings, approvals, calendar, events, meetings), not the pro set.
2. **┬¦9 production verification ŌĆö 0 gaps.** Wrote a dependency audit: every table/RPC referenced by each of the 19 `module_status.ready=true` modules was matched against `CREATE TABLE`/`CREATE FUNCTION` in migrations. **Zero references to undefined tables/RPCs.** Inventory verified separately (uses chained `.from()` calls). All 19 ready modules genuinely persist end-to-end. The "exists ŌēĀ production" gap from the audit is CLOSED for ready modules.
3. **Named bugs:** the `payments` route collision the audit flagged is **stale/resolved** ŌĆö only one `payments` route exists now (properly gated). Found a `settings/profile` duplicate path but it's in different route trees (top-level redirect vs nested `/app/`) ŌĆö harmless. No action needed.
4. **`PRODUCTION_REGISTER.md` produced** ŌĆö the verified ┬¦9 register artifact, cross-referencing features.ts + module_status + the audit result. This is the "documentation exercise that removes the single biggest risk."

### Applied Intelligence layer (deterministic, no LLM) ŌĆö the tier to sell before the copilot
...
- `intelligence_process_bottlenecks` ŌĆö stagnant deals (>14d) + stale tasks (>7d), severity-tiered.
- `intelligence_risk_anomalies` ŌĆö expense >2x historical avg, invoices to <24h-old contacts, payments reversed within 24h.
- `intelligence_capacity` ŌĆö staff workload vs business mean, labeled over/under-utilized.
- `intelligence_early_warnings` ŌĆö 3+ invoices overdue >30d, budgets at 90% consumed.
- `intelligence_sales_performance` ŌĆö sales targets vs closed-won attainment.
- `intelligence_cashflow_forecast` ŌĆö 90-day moving average projection (classical time-series; the narrative "why" stays with the generative copilot, Phase 3).
...
- `usage_events` table (migration `20260101000007`) ŌĆö append-only, RLS lets a business read/write only its own. `usage_module_adoption(business_id)` RPC for per-business adoption; `usage_cross_business_adoption()` for the builder dashboard (service-role only).
- `useUsageTracking` hook wired into Shell.tsx ŌĆö logs view events fire-and-forget on every route change (never blocks UX). Purpose: empirical "which of the 61 L2 modules actually get touched" data for sprint decisions ŌĆö independent of entitlements.
...
- **Phase 1 (done):** Applied Intelligence (deterministic) ŌĆö sellable Intelligence tier, no LLM dependency.
- **Phase 1 (done):** Usage telemetry infra.
- **NOT started, deferred to Phase 3:** Generative AI Copilot ŌĆö only after core ERP modules have real paying customers and real transaction history. Scoped to answer only from verified data (Fact-vs-Inference protocol). Do NOT build this on partially-fake modules.

## Session 10 (2026-08-12): autonomous self-contained audit ŌĆö DISCOVERY (read-only, no edits yet)
...
- Stack: Vite 8 + React 19 + TS 6, Tailwind v4, Supabase (Postgres + RLS + Edge Functions). No dedicated backend server; SPA ŌåÆ Postgres via Supabase SDK; RLS is the authorization boundary. 10 edge functions (paystack/flutterwave init+verify+webhook, subscription-management, send-email/sms/whatsapp/signature-request/welcome, dispatch-webhooks, execute-automation, parse-intent, transcribe-audio).
- Auth: Supabase Auth (email/password + OAuth). `AuthContext` maps sessionŌåÆ`staff` row. Client-side `permissions.ts`/`hasPermission` = UX gating only; RLS = real boundary.
...
### Critical findings (ranked SECURITY ŌåÆ DATA INTEGRITY ŌåÆ CORE ŌåÆ BUSINESS ŌåÆ PERFORMANCE ŌåÆ RELIABILITY ŌåÆ MAINTAINABILITY ŌåÆ DEAD WEIGHT)

**CRITICAL ŌĆö SECURITY**
1. **2FA is set up but NEVER enforced at login.** `SecuritySettings.tsx` lets users enable TOTP (generates secret, verifies code client-side, stores to `user_mfa`), but `Login.tsx`/`AuthContext.tsx` have ZERO MFA challenge logic ŌĆö `signInWithPassword` succeeds and the user is fully authenticated regardless of whether 2FA is enabled. 2FA is security theater: users believe they're protected, but a stolen password bypasses it entirely. This is the headline defect.
...
**HIGH ŌĆö DATA INTEGRITY / RELIABILITY**
...
7. **`getBalance` fetch omits the api_key** (`smsService.ts:252`) ŌĆö calls Termii `/sms/balance` with no `api_key` in headers/body, so it always fails. Dead/broken feature surfacing as "failed to load balance."

**MEDIUM ŌĆö DEAD SECURITY INFRASTRUCTURE**
8. **`999_security_fixes.sql` rate-limiting + API-key-usage + security-audit functions are NEVER CALLED by the app.** `check_auth_rate_limit`, `check_api_key_usage`, `log_security_event` have zero callers in `src/`. Login/signup have no rate limiting (rely only on Supabase's built-in). API keys are created (hashed correctly in `api_keys.key_hash`) but there is NO edge function / API gateway that validates a presented key against the hash ŌĆö so created API keys are unusable (matches `module_status.api = not ready`). Security audit events aren't logged via the dedicated function.

**LOW ŌĆö DEAD WEIGHT / HYGIENE**
...
11. **`WhatsAppBusiness.*` direct-API methods (`getConfig`/`send`/`sendTemplate`) are dormant** ŌĆö no page calls them (only `WhatsAppWeb.openChat` is used in NotificationsCenter). They're the plaintext-token-fetching path; leaving them invites future misuse. Either remove or route through the existing `send-whatsapp` edge function.

### Areas requiring deeper investigation (Phase 2 batches)
- Confirm whether ANY login flow checks `user_mfa.enabled` (none found ŌĆö confirms #1).
...
- **S1 (security, internal):** Enforce 2FA at login ŌĆö read `user_mfa` after `signInWithPassword`; if `enabled && method==='totp'`, intercept and challenge the TOTP code (verify client-side with `otpauth` ŌĆö already a dependency ŌĆö then set a "mfa-verified" session flag before app mount). No new dependency.
- **S2 (security, internal):** Hash `totp_secret`? NO ŌĆö TOTP secrets cannot be hashed (server must reproduce codes); correct internal fix is encryption-at-rest via a Postgres `pgcrypto` column encryption OR keep plaintext but accept it (row is user-scoped). Per ┬¦22, do not reinvent crypto; the pragmatic internal fix is to keep the secret but gate it behind the MFA-enforcement (S1) so the feature actually protects, and document the plaintext limitation. Backup codes: store hashed (SHA-256) matching the existing `backup_codes_hash` column + add a verify path. Fixes #2/#3.
...
- Defer #8 (dead security infra wiring) ŌĆö wiring `check_auth_rate_limit`/`log_security_event` into login is valuable but is a larger change; flag as a follow-up, not Phase 2, unless time permits. Per protocol ┬¦35, every fix batch ends with tsc + vite build + vitest green.


## Session 10 (2026-08-12): autonomous full-stack audit ŌĆö security/data/reliability/dead-code/dependency repair
...
### Critical (CLOSED)
- **Cross-tenant RLS leak (systemic, ~111 policies).** Policies used `business_id IN (SELECT id FROM businesses)` ŌĆö a subquery over the whole businesses table ŌĆö true for ANY authenticated staff member regardless of tenant. Effectively USING(true) on tenant-scoped tables: any user could read/write any tenant's data. **Migration 080** rewrites all 111 to `business_id IN (SELECT business_id FROM get_current_staff())`, adds missing RLS on asset_categories/expense_categories/entity_freshness view/business_events/approval_requests view, hardens approval_requests + entity_freshness_status views with security_barrier=true. Commit 3ed5a7a.
- **MFA bypass + unhashed backup codes.** 30bf3c8. Correct TOTP verify via otpauth; backup codes hashed, constant-time compare.

### High (CLOSED)
- **SMSBroadcast provider API-key leak** (secret sent to browser) + **edge-function auth** (anon/service key used without verifying caller JWT). 402b953 ŌĆö secret stays server-side; functions verify caller JWT before using service role.
- **.env.example live credentials.** 8bed327 sanitized.
- **Provider secrets loaded to client** (settings returned full secret values). 37750db ŌĆö stop loading secrets client-side; harden settings RLS.
...
- **useRealtime stale channel ref** on cleanup. 9f8a061 ŌĆö null the ref.
...
- **WhatsApp token:** masked (password + eye toggle), admin-only, RLS-gated; sendMessage only queues a DB row ŌĆö browser never calls Meta Graph with the token.
- **Realtime subscriptions:** all have `return () => supabase.removeChannel(channel)` cleanup.
- **Runtime deps (10):** all justified ŌĆö gsap (Landing.tsx only), jspdf (build-from-within PDF), otpauth (MFA), Sentry (opt-in), rest essential.

### Architectural risks (documented, not fixed ŌĆö feature-sized)
...
2. **Duplicate EmptyState pair** (EmptyState.tsx default + EmptyStates.tsx named). Not merged ŌĆö would touch 9+ pages with visual-regression risk.
3. **useModuleAccess ineffective dynamic import** (build warning) ŌĆö deliberate (avoids AuthContext<->useModuleAccess circular dep, Session 8); cosmetic.
...
- **Deploy note:** migration 083 must be applied to live Supabase for the new columns + RPC signature. Before it's applied: Profile silently ignores the new fields on save (PostgREST drops unknown columns), onboarding falls back to 'Owner' ŌĆö no crash. Migration 082 (self-audit) and 080/081 (RLS/FK) from prior sessions are also still pending on the live DB.


### NotificationBell realtime crash (committed 845d98a, deployed)
- `Shell.tsx` renders `<NotificationBell />` TWICE ŌĆö line 433 desktop header + line 449 mobile header ŌĆö and Tailwind `md:hidden`/`hidden md:flex` only toggles CSS visibility, NOT React mounting. Both instances always mounted and both subscribed to the same hardcoded `supabase.channel('notifications:realtime')`. Supabase client dedupes by name ŌåÆ returns the cached channel ŌåÆ second `.on().subscribe()` throws "cannot add callbacks after subscribe()" ŌåÆ page crash.
- Fix: per-mount random suffix on the channel name (`notifications:realtime:${Math.random().toString(36).slice(2)}`). Complements the earlier dep-array/ref fix (a different contributor ŌĆö teardown/recreate churn). Both needed.
- `polls` "Batch 4" was correctly SKIPPED: the DB migration already defines `closes_at` (not `ends_at`); `CompanyHome.tsx` already selects `closes_at` and already normalizes both `options` shapes (`opt.text || opt`). The "ends_at ŌåÆ closes_at" concern was based on a stale read. Lesson: read live code, not screenshots, before writing SQL.

### Mobile app (Android + iOS) ŌĆö BUILD NOW WORKING (committed dd37c45)
...
- **Root cause:** version skew. `@expo/vector-icons@14.1.0` depends on `expo-font@57.0.1` (future SDK), while `expo@51` depends on `expo-font@12.0.10`. npm hoisted `expo-font@57.0.1` to top-level; its build.gradle uses the NEW `plugins { id 'expo-module-gradle-plugin' }` mechanism that SDK 51's `settings.gradle` `useExpoModules()` does NOT register ŌåÆ plugin not found.
...
- **CI result (run 31786545369):** ALL GREEN ŌĆö Mobile Type Check Ō£ō (31s), Android APK Ō£ō (8m41s), iOS App Ō£ō (13m41s). Artifacts: `avenize-android` (APK), `avenize-ios` (.app simulator build). Download: `gh run download 31786545369 --repo akinsify00511-cpu/fabric -n avenize-android`.
- **Limitations of the self-hosted CI build:** the APK is UNSIGNED (no keystore configured) ŌĆö installable on Android with "install unknown apps" but not Play Store ready. The iOS build is a SIMULATOR build (`CODE_SIGNING_ALLOWED=NO`) ŌĆö runs in iOS Simulator, NOT installable on real iPhones. For real device/Store distribution you need EAS Build (an Expo account + `EXPO_TOKEN` secret in GitHub) or local signed builds (keystore for Android, Apple Developer account + signing certs for iOS).

### Mobile build path DECISION (2026-08-14, commit 87c861d): GitHub Actions only, NOT EAS cloud
- **Decision:** retain the mobile build pipeline within GitHub Actions (`mobile.yml`); do NOT use EAS/Expo cloud builds. The Expo *framework* stays (the app is built on `expo` + `react-native` + `expo prebuild`), but the build pipeline is GitHub Actions producing downloadable artifacts ŌĆö no Expo account, no EXPO_TOKEN, no EAS dashboard.
- **Removed** `mobile/eas.json` (was unused by the GitHub workflow ŌĆö which runs `npx expo prebuild` + gradle/xcodebuild locally in CI ŌĆö and its presence caused "Failed to read /eas.json" confusion when commands were run from the repo root). **Cleared** `app.json extra.eas.projectId` back to empty.
- **Verified post-removal (run 31807162102):** ALL GREEN ŌĆö Mobile Type Check Ō£ō (29s), Android APK Ō£ō (7m57s), iOS App Ō£ō (15m26s). The GitHub-only path is solid and does not depend on any Expo/EAS config.
- **What the GitHub build produces (final state):**
  ...
  - iOS: simulator build (`CODE_SIGNING_ALLOWED=NO`, `-sdk iphonesimulator`). Runs in iOS Simulator only. NOT installable on real iPhones ŌĆö real-device iOS signing in CI would require Apple Developer cert + provisioning profile secrets in GitHub, which is the hard constraint of the no-EAS path.
  - Supabase config flows from GitHub secrets: workflow uses `secrets.EXPO_PUBLIC_SUPABASE_URL || secrets.VITE_SUPABASE_URL` (and the ANON_KEY equivalent), so the mobile app reuses the same secrets the web deploy uses ŌĆö no Expo dashboard env vars needed.
...
- Real app, not a stub: Login (Supabase Auth + SecureStore token persistence), Capture (parse-intent edge fn ŌåÆ raise business event), Observer (org snapshot w/ pull-to-refresh), More (profile + module links). AuthContext mirrors web. 5 screens total ŌĆö Capture/Snapshot/Tasks/Chat/More tabs (Tasks/Chat are placeholders). Shares brand tokens with web. Typechecks clean.
...
## Session 13 (2026-08-14): Intelligence Transformation ŌĆö P1 activation (U1ŌĆōU6)

Triggered by the master "Business Intelligence Transformation" directive. Per ┬¦41: understand first ŌåÆ analysis report ŌåÆ wait for approval ŌåÆ implement. Report (`AVENIZE_INTELLIGENCE_CURRENT_STATE.md`, sections AŌĆōV) written and approved ("yes proceed"). P1 then executed incrementally; baseline held green after every step. Final: `tsc -b --noEmit` clean, `vite build` succeeds, `vitest run` 73/73 (was 61, +12 new). All changes internal SQL + a thin client wrapper; no working module rewritten; no external dependency.

### Drift discovered and FIXED (not papered over) ŌĆö the highest-risk defect class
- **`invoices` has NO `contact_id`** (001/002 use `client_name`/`client_email`/`deal_id`), yet the `ŌĆ”06` intelligence family + 068 self-audit + ŌĆ”06 referenced `invoices.contact_id` ŌåÆ would error on a live DB. Governed metric engine (086) uses real `client_name` for customer attribution.
- **`deals` uses `stage` (won/lost/ŌĆ”), `owner_id` (NOT `assigned_to`), and has NO `closed_at`.** The 059 `emit_deal_won` trigger checked `NEW.status = 'closed_won'` ŌåÆ **DealWon NEVER fired.** Fixed in 090 (fires on `stage`ŌåÆwon). The entire `ŌĆ”06` family referenced non-existent `deals.assigned_to`/`closed_at` ŌåÆ confirmed dead+broken, deprecated.
- **`tasks` uses `assignee_id`** (not `assigned_to`). Data-quality scanner (089) uses the real column.

### What shipped (6 idempotent migrations 085ŌĆō090, ~1,574 lines)
- **U1 (085): consolidate duplicate intelligence RPC families.** 063 family is canonical (consumed by IntelligenceHub/ExecutiveCockpit/MarketIndex). The `20260101000006_applied_intelligence.sql` `intelligence_*` twins have ZERO callers AND are drifted/broken ŌĆö deprecated (kept callable one release via COMMENT, not DROP). Added `sales_performance_intelligence` (replaces a twin that referenced non-existent `sales_targets.target_amount`; real col is `revenue_target`) + `cashflow_forecast_intelligence` (90d moving avg with explicit insufficient-data guard, ┬¦21). JSONB house style.
- **U2 (086): metric registry + governed engine.** `metric_definitions` (┬¦7) seeded with 20 governed metrics ŌĆö each has definition/formula/sources/period/min_sample/insufficient_note. `refresh_business_metrics(business_id)` is the ONLY writer of governed `kpi_metrics` rows; emits `sample_size`+`confidence` (high/medium/low/insufficient); value is NULL below min_sample (┬¦21). `current_metrics` read helper. `kpi_metrics` (019, dormant shell) extended additively + activated. `GovernedMetricsCard` in ExecutiveCockpit surfaces real numbers, change %, confidence, and the honest "insufficient data" note ŌĆö never a fabricated value.
- **U3 (087): wire the context graph.** New best-effort handler `handler_derive_relationships` registered with the event bus at run_order 6 (after propagation at 5, before freshness at 10) derives CustomerŌåÆDealŌåÆInvoiceŌåÆPayment edges via the existing `link_entities` (060). Existing triggers/handlers untouched. `business_relationships` read helper over `recursive_neighbors` (060) for impact analysis.
- **U4 (088): recommendation + outcome loop.** Extended `claims` (060) additively with lifecycle columns (status/rule_id/severity/owner_id/action_type/linked_action_id/expected_impact/actual_impact). Lifecycle RPCs: `acknowledge_recommendation`, `set_recommendation_decision`, `mark_recommendation_acted`, `record_recommendation_outcome` (computes accuracy vs expected). `recommendation_effectiveness` (┬¦16 by-rule historical success) + `open_recommendations` (┬¦17 severity-prioritised feed). A recommendation IS a `claims` row ŌĆö no parallel `recommendations` table.
- **U5 (089): data-quality scanner.** `scan_data_quality` set-based scanner: orphaned invoice, missing due date, negative amounts, deal w/o owner, unassigned task, stale entity (via `entity_freshness_status` VIEW), duplicate contact, unreconciled payment. Each check in its own EXCEPTION block (┬¦24). Writes findings into `self_audit_findings` (audit_dimension CHECK extended to `data_quality`) + summary into `data_quality_checks`. Never mutates business data (┬¦14) ŌĆö advisory only. Idempotent via unique indexes.
...
### Tests (┬¦29/┬¦30)
New `tests/frontend/lib/governedMetrics.test.ts` (12 tests): confidence contract (highŌåÆFACT, medium/lowŌåÆINFERENCE, insufficient/errorŌåÆUNKNOWN), small-data formatting (nullŌåÆ"ŌĆö", currency/percent/duration/number/ratio), and the recommendation lifecycle union asserted against the DB CHECK constraint.

### STILL PENDING ŌĆö needs live DB (U0, flagged to user)
- **Deploy migrations 080 + 081 + 082 + 083 + 085ŌĆō090 to live Supabase** (project kgsgqvatyleetyquffya). All idempotent (`CREATE OR REPLACE`/`IF NOT EXISTS`/`ON CONFLICT`), safe to apply. Until applied, new RPCs aren't callable ŌĆö but the frontend degrades gracefully (governed panel/recommendations/data-quality stay empty) because every caller is best-effort + non-blocking (┬¦24 safe-failure). No commit has been made this session yet.
- **pg_cron jobs** (once pg_cron enabled): `refresh_business_metrics(business_id)` per business + `detect_customer_inactive_all()` daily.
- Golden test datasets (┬¦30) + live DB failure testing (┬¦60/┬¦81) ŌĆö follow-ups needing the live DB.

### Deliberately NOT done (┬¦22/┬¦31/┬¦33)
- No external AI/analytics APIs ŌĆö all intelligence is deterministic SQL over real tables.
- No new modules, no chatbot, no superficial dashboards ŌĆö reuse ExecutiveCockpit + claims infra.
- No forecast narrative ("why") ŌĆö stays with a future generative layer (Phase 3); 085 only emits the deterministic number + assumptions.

## Session 13b (2026-08-14): Intelligence Transformation ŌĆö P2 activation (U7ŌĆōU10)
...
- **U9: Data Quality view page** (`src/pages/DataQuality.tsx`, route `/app/data-quality`, `self_audit` gate): surfaces `scan_data_quality` findings with severity stats, per-finding fix links to the source page (invoiceŌåÆfinance, dealŌåÆcrm, taskŌåÆtasksŌĆ”), and a resolve action. Advisory only ŌĆö never mutates business data (┬¦14). Wired into App.tsx routes, Shell ROUTE_MODULE map, and the Controls nav group.
- **U10: Recommendations feed in Executive Cockpit.** New `RecommendationsCard` ŌĆö the "What needs my attention" feed (┬¦17) ŌĆö surfaces `open_recommendations` with accept/reject/acknowledge actions and expected-impact. Honest empty state ("No open recommendations. As your business data growsŌĆ”") when no data yet. Best-effort: stays empty if the recommendation migration isn't deployed.

### Deploy status
- Vercel production: Ō£ģ deployed (be9fd20). Build + Deploy green.
- ŌÜĀ’ĖÅ STILL needs live DB: migrations **080ŌĆō092** must be applied to Supabase (project kgsgqvatyleetyquffya) for the new RPCs to take effect. Frontend degrades gracefully until then (panels empty, no errors ŌĆö every caller is best-effort/non-blocking, ┬¦24).

### Verification
tsc clean, vite build succeeds, vitest 73/73 (unchanged; no new tests this batch ŌĆö the issuer rules are SQL and need the live DB for golden-dataset validation per ┬¦30).

## Session 14 (2026-08-14): Intelligence Transformation ŌĆö Business Health + OKR + Risk + Nav + Trust (U11ŌĆōU15)
...
### U11 ŌĆö Business Health engine (┬¦21, the keystone)
- **093 migration:** `business_health_scores` table + `health_metric_map` (governed metric ŌåÆ dimension + direction + label + weight) + `compute_business_health(business_id)` RPC. The explainable, decomposable composite score: each dimension (financial, sales, customers, operations, people, projects) averages the sub-scores of governed metrics (086) that have BOTH a current_value AND a target_value. Higher-is-better: clamp(actual/target├Ś100). Lower-is-better (overdue %, collection period): clamp(1ŌłÆactual/target├Ś100). Dimensions with no target-backed data are flagged `insufficient_data` and excluded from the overall (honest, not a guess ŌĆö ┬¦21). DQ penalty: ŌłÆ2 per critical finding (max ŌłÆ10), ŌłÆ1 per warning (max ŌłÆ5) from 089 findings. Open critical recommendations (091) surfaced as a flag (no double-penalty ŌĆö the underlying metrics already reflect the condition). `dimension_scores` JSONB stores the full breakdown so the UI answers "why 81" with the actual numbers. `current_business_health` read helper. Granted to authenticated.
...
### U13 ŌĆö Risk register (┬¦48)
- **095 migration:** `business_risks` table. Categories (financial/customer/operational/project/people/strategic/compliance). probability + impact (1-5 each) with a trigger-computed `risk_score` (probability ├Ś impact, 1-25). Owner, mitigation plan + status, due date, lifecycle status, evidence JSONB, optional entity link. `risk_summary` RPC (counts + avg score by category). RLS via `get_current_staff`.
...
### Deploy status
- Vercel production: Ō£ģ all commits deploying. Builds green.
- ŌÜĀ’ĖÅ STILL needs live DB: migrations **080ŌĆō096** must be applied to Supabase (project kgsgqvatyleetyquffya). All idempotent. Frontend degrades gracefully until then (health card shows "set targets", OKR/Risk/Trust pages show empty/error states, recommendations empty) because every caller is best-effort/non-blocking (┬¦24).
...
- **U16 ŌĆö Monthly Performance Review (┬¦26):** migration `097_monthly_performance_review.sql` adds `monthly_review(business_id, period_start, period_end)` ŌĆö a read-only roll-up of Business Health (093) + OKR progress (094) + open Risks (095) + open Recommendations (091) + governed metric movers (086) + data-quality summary (089) for a month window. NO new tables ŌĆö pure interpretation over existing data (┬¦0/┬¦6). `MonthlyReview.tsx` page (`/app/review`, self_audit gate): month selector, summary header, per-dimension health breakdown, OKR progress, top risks + recommendations, metric movers, DQ counts. Board-ready + PRINTABLE (print CSS). ExecutiveCockpit's BusinessHealthCard deep-links to the full MPR. Deployed (9414ec1).
- **U17 ŌĆö Required documentation (┬¦35/┬¦36):** `METRIC_DICTIONARY.md` ŌĆö the canonical metric registry (┬¦6/┬¦7): all 21 governed metrics with key/name/formula/sources/unit/min_sample/status, the governance layers (metric_definitions ŌåÆ kpi_metrics ŌåÆ health_metric_map ŌåÆ refresh_business_metrics), and the ┬¦7 anti-patterns prevented. `INTELLIGENCE_RULE_CATALOG.md` ŌĆö every recommendation rule in the full ┬¦36 format (8 implemented: FIN-AR-001/002, FIN-CF-001, SAL-CONV-001, INV-001, CUST-001, OPS-001, DQ-001 + 8 planned as candidates only). Both verified against the real migrations, not written from memory. Committed 60e2253.
...
verified against real migrations ŌĆö never aspirational):
- AVENIZE_INTELLIGENCE_ARCHITECTURE.md ŌĆö top-level stack, outcome loop (┬¦15), failure isolation (┬¦24), performance (┬¦23), component map.
- BUSINESS_DATA_MODEL.md (┬¦3) ŌĆö 362 real tables mapped to canonical conceptual model; cross-module relationship graph (┬¦4) verified against FKs; no duplication (customers=contacts, revenue=derived metric, actions reuse tasks/approvals/POs, OKRs extended strategic_objectives).
- BUSINESS_EVENT_CATALOG.md (┬¦5) ŌĆö the single event bus (058/059/090): 10 emitted events + AI-captured + catalogued-handler + flywheel wiring. No competing architecture.
- DATA_QUALITY_MODEL.md (┬¦8) ŌĆö 089 scanner: 7 implemented checks, health-penalty + DQ-001 + MPR loop, and what it deliberately does NOT do (never mutates business data, ┬¦14).
- RECOMMENDATION_CATALOG.md (┬¦12/┬¦13) ŌĆö 8 recommendation types in full ┬¦12 field set + lifecycle + effectiveness (┬¦16). Specific to real data, never generic (┬¦13).
- INTELLIGENCE_ROADMAP.md (┬¦31/┬¦32) ŌĆö P0/P1/P2 done, P3 deferred, blocked-on-live-DB items, explicit anti-goals (┬¦33).
- INTELLIGENCE_TEST_MATRIX.md (┬¦29/┬¦30) ŌĆö unit coverage, SQL scenarios with expected behavior, 7 golden datasets (┬¦30), live failure testing (┬¦60/┬¦81).
...
- `supabase/tests/golden_dataset_validation.sql`: the ┬¦30 validation runner ŌĆö seeds all profiles, runs the intelligence engine, SELECTs the per-profile result summary with expected values as comments for assertion. Cleans up after itself.
...
Three directive gaps closed ŌĆö all frontend/internal, no new tables, no external deps.

- **┬¦19 evidence drill-down (commit a91cbd1):** the Cockpit/MPR showed the recommendation statement + expected impact but never the underlying evidence JSONB. New reusable `EvidencePanel` in Evidence.tsx renders the evidence object in a human-readable key/value form (amount keys ŌåÆ Ōé”, ratios ŌåÆ %, booleans ŌåÆ yes/no). Each recommendation now has an expandable "Why are you telling me this?" toggle on both the Executive Cockpit and the Monthly Performance Review. Migration 097's `monthly_review` RPC now returns `evidence` in each recommendation object (idempotent CREATE OR REPLACE).
- **┬¦16 recommendation effectiveness (commit 02f6b0d):** `fetchRecommendationEffectiveness` + the `recommendation_effectiveness` RPC (088) had ZERO callers ŌĆö the learning loop was built but never surfaced. New `EffectivenessCard` on the Cockpit ("Did the recommendations work?"): per rule ŌĆö issued / accepted / acted / outcomes (with success count) / avg expected vs avg actual impact. Closes ┬¦39 board question #5.
- **┬¦14 action layer (commit 0934f8c):** the outcome-loop RPCs existed but the frontend only flipped claim status ŌĆö no path from "accepted" to a real task. Accepted recommendations now show an "Act ŌåÆ Create task" button that inserts a real `tasks` row + calls `mark_recommendation_acted` (statusŌåÆacted, linked_action_id set). Reuses existing tasks table ŌĆö no new action system (┬¦14). Closes the ┬¦15 outcome loop on the frontend.
...
## Session 15 (2026-08-15): Infrastructure tightening ŌĆö "one organ, brain coordinating a body"

User directive: tighten infrastructure so the app works as one organism ŌĆö a brain (central event bus) coordinating a body (modules). 5 batches, each committed + verified before the next. All pushed; CI green; Vercel deployed (commit ceca6c9, run 31881465078).

### Batch A ŌĆö Repair migration spine (93ad97b)
- Found 8 duplicate-numbered migration files (023, 031, 032, 039, 041, 042, 082, 999 each had TWO files at the same number). Initial approach: renumber second-of-pair to 100-series. **This was REVERTED in Batch C** ŌĆö see below.
- Removed `999_grant_scale_access.sql` (hardcoded a real person's email+UUIDs to grant owner/admin ŌĆö entitlement granting belongs in the app flow, not schema history).
- Narrowed `998`'s blanket `GRANT EXECUTE ON ALL FUNCTIONS TO anon` ŌåÆ authenticated-only (anon keeps explicit grants from 050/053). Dropped TRUNCATE from table verbs.
- Added `108_schema_version_tracking.sql`: `db_schema_version()`/`db_is_current()` so the app can verify DB matches codebase.

### Batch B ŌĆö Complete the event-bus nerve map (674e667)
The event bus (058) is the nervous system. Audit of 059/090 found events cataloged but never actually fired:
- **FIXED InventoryLow (drifted):** `emit_inventory_low` read `reorder_level`/`reorder_point`, but `products` uses `low_stock_threshold` (001) ŌåÆ `v_reorder` was always 0 ŌåÆ guard failed ŌåÆ NEVER fired. Now reads the real column with a fallback chain covering both stock models (`products.low_stock_threshold`, `inventory.reorder_level`). Added re-emission guard (only fires on threshold crossing, not every stock touch).
- **FIXED EmployeeExited (drifted):** only fired on a staff `status` column that doesn't exist (staff uses `active` BOOLEAN, 002). Now fires on `active` trueŌåÆfalse, with a status-based fallback.
- **ADDED CampaignConverted:** trigger on `email_campaigns` statusŌåÆsent.
...
### Batch C ŌĆö CI hardening + Batch A regression fix (f58ef97)
...
- **CI migration-apply gate (the real tighten):** the old `database-tests` job swallowed every error with `|| true` and skipped all assertions ŌĆö a broken migration shipped green. Replaced with:
  ...
  - Apply every migration with `ON_ERROR_STOP=1` (no `|| true`). `continue-on-error: true` on the apply step because ~54 historical migrations have pre-existing apply issues (non-idempotent statements, fresh-DB ordering) ŌĆö the step's value is the FAILURE REPORT (catches new drift + tracks baseline), not a hard gate.
...
- **Drift fixes caught by the new gate:** 109 nested dollar-quoting in pg_cron block (`$$` inside `DO $$` ŌåÆ use `$_$`); 082 missing `DROP CONSTRAINT IF EXISTS` before `deals_stage_check`; 022 missing `DROP POLICY IF EXISTS` before branding policies.
- **CI result:** 58 applied / 54 failed (historical baseline), smoke test PASSED, build green. The 54 historical failures are documented as known debt ŌĆö the goal is to not INCREASE this count; new migrations that fail to apply are now immediately visible.

### Batch D ŌĆö Realtime business pulse (ceca6c9)
- The event bus wrote to `business_events` and 082 added it to `supabase_realtime` ŌĆö but nothing in the app LISTENED. The brain was deaf.
...
### Deploy status
- Vercel production: Ō£ģ deployed (ceca6c9). Build + Deploy green.
- ŌÜĀ’ĖÅ STILL needs live DB: migrations **080ŌĆō109** must be applied to Supabase (project kgsgqvatyleetyquffya). All idempotent. Frontend degrades gracefully until then (pulse shows no events, detectors return 0, recommendations empty) because every caller is best-effort/non-blocking.
- **CI baseline established:** 58/112 migrations apply clean against bare postgres:15; 54 have historical drift (non-idempotent statements, fresh-DB ordering dependencies). The new CI gate reports these visibly and catches NEW drift. Fixing the 54 is a follow-up that needs the live DB to resolve (many are cascading ŌĆö an early migration fails, so later ones that depend on its tables also fail).

## Session 15b (2026-08-15): Supabase reconciliation ŌĆö database contract + schema-drift CI gate
...
- Built `scripts/generate_reconciliation_matrix.py` ŌĆö cross-references frontend `.from()` references (202 tables, excluding storage buckets) against migration `CREATE TABLE`/`CREATE VIEW` (393 tables+views) + `CREATE FUNCTION` (RPCs).
- **Class F (drift ŌĆö frontend references a table with NO backing migration) = ZERO.** Every frontend table reference has a backing migration (CREATE TABLE or CREATE VIEW). Every frontend RPC (4) has a backing CREATE FUNCTION. Every storage bucket (3: avatars, documents, signatures) has a backing `INSERT INTO storage.buckets`.
- The "116 missing tables" gap (frontend 204 vs live 88) is **DEPLOYMENT drift** ŌĆö migrations exist in Git but haven't been applied to the live Supabase (project `kgsgqvatyleetyquffya`). The fix is to apply pending migrations, NOT to write 116 new ones. This is a critical distinction that changes the entire remediation strategy.
- Classification: A=48 (Layer 1 required), B=16 (intelligence/event infra ŌĆö preserve), C=138 (future/extended feature), D=0, E=0, F=0.

### Deliverables
- **`supabase/reconciliation/RECONCILIATION_MATRIX.md`** ŌĆö the A-F classified matrix for all 202 frontend-referenced tables, with migration sources, consumer counts, and Layer 1 / intelligence flags.
- **`supabase/reconciliation/schema_manifest.json`** ŌĆö the machine-readable database contract (table, classification, has_migration, migration_sources, layer1, is_intelligence, frontend_consumers, deployment_status=UNKNOWN until live DB checked).
- **`supabase/reconciliation/LAYER1_CANONICAL_SCHEMA.md`** ŌĆö explicitly defines the 48 Layer 1 tables (CRM/Sales 16, Inventory 12, Accounting 18, HR 18, Core 10 ŌĆö some overlap) with purpose + migration source for each. Documents the source-of-truth hierarchy, the deployment gate, and the intelligence tables that must be preserved. Lists what needs live DB access.
- **`scripts/check_schema_drift.py`** ŌĆö the CI drift detector. Scans frontend for `.from()`, `.rpc()`, `.storage.from()` references; verifies each has a backing migration; exits 1 if any reference is unbacked. Excludes `storage.from()` (storage buckets aren't table queries) and catches `CREATE VIEW` (not just `CREATE TABLE`).
- **CI job `schema-drift`** ŌĆö runs on every PR; build now `needs: [typecheck, unit-tests, database-tests, schema-drift]`. A new page querying a table with no migration fails CI before merge.

### Intelligence work verified intact
The reconciliation explicitly verified that all Session 13ŌĆō14 intelligence/event tables (business_events, claims, kpi_metrics, business_health_scores, key_results, business_risks, self_audit_findings, action_reversals, etc.) are classified B (preserve) and have backing migrations. The recommendation loop (Insight ŌåÆ Recommendation ŌåÆ Accept ŌåÆ Action ŌåÆ Outcome ŌåÆ Effectiveness) remains intact.

### Deploy status
- Vercel production: Ō£ģ deployed (33d041b). Build + Deploy green.
- CI: Ō£ģ all jobs green ŌĆö Type Check, Schema Drift Check (0 drift), Unit Tests (73/73), Migrations Apply Clean (58 applied / 54 historical baseline, smoke test passed), Build.
- ŌÜĀ’ĖÅ Still needs live DB: apply migrations 080ŌĆō109, verify RLS, tenant-isolation testing, auth chain verification. All documented in LAYER1_CANONICAL_SCHEMA.md.
...
### LandingEnhanced (marketing homepage) ŌĆö full rewrite
- New hero: "More capable than an ERP. Easier than WhatsApp." with a live "My Work" preview card showing "Good morning David, 3 things need you" ŌĆö the actual product concept.
...
### CompanyHome (app homepage = /app) ŌĆö transformed
...
## Session 18 (2026-08-15): Lighthouse Performance CI green ŌĆö accessibility 100, performance 85
...
- **lighthouserc.json:** `urls` changed from `http://localhost/index.html` ŌåÆ `http://localhost/`. The SPA router saw path `/index.html`, didn't match route `/` ŌåÆ rendered NotFound 404 page. Lighthouse was auditing the 404 page (wrong content, missing `<main>`, low-contrast links) instead of LandingEnhanced.
- **LandingEnhanced.tsx:** BRAND colors darkened to pass WCAG AA (4.5:1) even on soft-tint backgrounds (badges with `rgba(color, 0.08)` backgrounds). `primary` #4285F4ŌåÆ#155BB4 (6.3:1 on white + soft-blue), `amber` #E89400ŌåÆ#845400 (5.2:1), `success` #1E8E3EŌåÆ#157342 (5:1), `textMuted` #9AA0A6ŌåÆ#5F6368 (7:1). Added `<main>` landmark wrapper. Mobile menu button got `aria-label`.
...
- **NotFound.tsx:** added `<main>` landmark; `/app` link `text-[#4285F4]`ŌåÆ`#1B6FE0`; secondary text `text-black`ŌåÆ`#5F6368`.
- **SarahChat.tsx:** floating help button got `aria-label="Open help guide"`.
- **index.html:** Google Analytics deferred (`async`ŌåÆ`defer`); fonts async-loaded via `preload` + `media="print" onload="this.media='all'"` trick (non-blocking).
- **PDFGenerator.ts:** `jspdf` + `jspdf-autotable` changed to dynamic `await import()` so the 137KB-gzip PDF library only loads when a user generates a PDF ŌĆö not on every page load.
...
- CI: Lighthouse Performance job Ō£ģ SUCCESS (only warn-level render-blocking + text-compression, no error-level failures).
- CI: UX Tests (Mobile) Ō£ģ, UX Tests (Desktop) Ō£ģ (all: accessibility, keyboard, error, tap target, empty state, visual regression, gap-fill modules, module gate).
- Vercel deploy Ō£ģ.
...
- **PR #12 MERGED** (fix/onboarding-loop-pricing-tiers-ci): onboarding loop fix + 5-tier pricing + Deploy Preview CI fix (replaced broken `amondnet/vercel-action@v25` with direct `npm i -g vercel` CLI). This was the blocker ŌĆö its deploy.yml fix unblocked every other PR's Deploy Preview check.
- **PR #10 MERGED** (fix/service-worker-release-cache): isolated `public/sw.js` change, zero file overlap. Used GitHub's `update-branch` API (PUT /pulls/{n}/update-branch) to merge main into the PR branch server-side (shallow clone can't merge locally ŌĆö "unrelated histories"). All checks passed post-update.
...
- **PR #7 CLOSED** (feat/fabric-workspace-phase2): DUPLICATE dashboard; main's Dashboard already implements the same `Representation` type + `RepresentationPicker`. Also failed CI (TS error: `'Icon' cannot be used as a JSX component` at FabricWorkspace.tsx:119 ŌĆö tuple-array type inference).
- **PR #5 CLOSED** (fix/production-ci-current): stale, would REVERT main work (migration 110, RepresentationEngine+tests, Landing/Pricing redesigns). Its one unique fix ŌĆö `send-email-notification` recipient rename ŌĆö was already on main. CI intent superseded by #12.
...
Systematic scan: `grep -rhoE "\.rpc\('[a-z_]+'" src/` ŌåÆ compare caller params against `CREATE FUNCTION` signatures in migrations. Found 4 flagged; 2 real bugs, 2 false positives (regex artifacts from multi-line signatures / nested Object.fromEntries).
...
Probed the live Supabase (project `kgsgqvatyleetyquffya`) directly via REST API using the publishable key extracted from the deployed JS bundle (new `sb_publishable_` format, not a JWT ŌĆö found via `grep -oE '.{0,50}kgsgqvatyleetyquffya.supabase.co.{0,200}'` on the index chunk).
- **Method to distinguish truly-missing vs signature-mismatch:** call the RPC with `{}` and check the error `details`. `"no matches found in the schema cache"` = function truly doesn't exist. `"Searched for the function with parameter X"` = exists but wrong args (or empty-body test artifact). The empty-body test alone is unreliable ŌĆö it returns PGRST202 for ANY function requiring params.
...
- **Root cause:** migrations `063` + `080`ŌĆō`110` have NOT been applied to the live database. This is deployment drift, not a code bug.
- **Impact:** `create_business_and_owner` missing = NEW USERS CANNOT ONBOARD (Onboarding.tsx shows "business creation service is not yet configured" ŌĆö graceful but broken). `can_access_module` missing = module gate treats unknowns as not-ready (safe-closed, but most modules hidden). Entire intelligence layer non-functional.
- **Frontend already degrades gracefully** for most (best-effort empty states per ┬¦24), but some pages surface raw PostgREST errors (MarketIndex was one ŌĆö now fixed).
- **BLOCKED on user action:** the user must apply pending migrations (`063` + `080`ŌĆō`110`) to the live Supabase via `supabase db push` or the dashboard. This cannot be done from the codebase ŌĆö requires DB credentials/service-role key. This is the single highest-priority deployment action.
...
- **Adaptive data fetching:** only fetches tables for active tools ŌĆö a user without inventory doesn't pay the products query cost. Tasks + staff always fetched (core attention + company-size signal).
...
- **Company-size complexity:** `isSolo` (people <= 1) hides the People card ŌĆö a one-person business doesn't need a headcount KPI. This is the first step of the PRD's progressive-complexity requirement; the full solo/team/enterprise tier system is P1 #7 (follow-up).
...
- **`src/lib/useExperienceContext.ts`:** `useExperienceContext()` COMPOSES the existing access hooks (entitled+role tools via `useAccessibleTools`, workspace selection via `useWorkspaceSelection`) instead of re-fetching them, and adds the three signals that were previously missing or computed ad-hoc per screen:
  - **industry** ŌĆö fetched from `businesses.industry` (was never loaded client-side before; Onboarding writes it but nothing read it back).
  - **companySize** ŌĆö authoritative headcount via one `staff` count query, centralizing what Dashboard computed inline. Module-level cached (`businessProfileCache`) so Shell + Dashboard + future screens share ONE fetch; cleared on signOut (`clearExperienceContextCache` wired into `AuthContext.signOut`).
  - **complexity** ŌĆö `deriveComplexity(companySize, activeModuleCount)` returns the PRD's 4 progressive-complexity tiers: solo (<=1) / small (2-10, <8 modules) / mid (11-50, or 2-10 with >=8 modules) / enterprise (>50). The UI reveals more machinery (departments/approvals/budgeting/compliance) as a business grows ŌĆö no manual toggle. Pure function, unit-tested.
  - **activeTools** ŌĆö the INTERSECTION of authorized Ōł® selected (or all authorized if not curated). Selection can never ADD an unauthorized tool. Exposes `isToolActive`/`isToolAuthorized` helpers.
- **Consumers refactored to the single source:**
  ...
  - `Shell.tsx` `itemVisible` now uses `isToolAuthorized`/`isToolActive` from the context (removed its `useAccessibleTools` import; module gate stays on `useAccessibleModules` ŌĆö it's a business-level entitlement+readiness check, not a per-user signal).
...
The 401 was NOT just an auth-timing issue (the earlier "half-fix" added auth-lifecycle gating to eventTracker ŌĆö necessary but insufficient). Root cause was a THREE-WAY RPC + schema conflict:
...
- 037 created `record_analytics_event(p_category event_category)` expecting `category event_category` / `user_id` / `component` / `action` / `metadata` columns ŌĆö but 037's `CREATE TABLE IF NOT EXISTS` was a no-op (019 won), so the table never had those columns. Its `CREATE TYPE event_category` may also have failed on the live DB (037 is one of the un-applied migrations).
...
- The live client (`eventTracker.ts`) called the 037 signature with named args. On the live DB, whichever `CREATE OR REPLACE FUNCTION` applied last won ŌĆö and if the enum type didn't exist, the function couldn't be created ŌåÆ PostgREST returned function-not-found, surfaced to the browser as a 401. The eventTracker's error-catch list didn't include `PGRST202` (function not found), so events were re-queued and retried forever (unbounded queue growth) ŌĆö the "401 that never stopped".

**Migration 111 (`111_analytics_events_reconciliation.sql`):** normalizes the table to the columns the live caller writes/reads (`user_id, category, component, action, metadata, page, session_id` ŌĆö added via `ADD COLUMN IF NOT EXISTS`, legacy 019 columns kept + backfilled); drops ALL conflicting `record_analytics_event` overloads (enumerated + a `pg_proc` sweep, guarded so it doesn't error if the `event_category` enum type never got created); creates ONE canonical `record_analytics_event` with `p_category TEXT` (not the enum ŌĆö eliminates the signature-mismatch + missing-type failure mode), `SECURITY DEFINER` (bypasses the SELECT-only RLS), granted to `authenticated`; adds an INSERT policy keyed on the caller's business; drops the unused 019 `track_event`.

**Client hardening (`eventTracker.ts`):** the catch now distinguishes "permanently unavailable" (function-not-found `PGRST202`, no-schema-cache-match, permission-denied, does-not-exist) ŌåÆ DROP the batch (analytics is optional, don't grow the queue) vs "transient" (network/timeout) ŌåÆ re-queue. Adds `PGRST202`/`42P01`/`42804` + message-pattern matching. This is the "completion" ŌĆö even if migration 111 isn't applied yet, the client no longer retries forever.

**One insert path (`analytics.tsx`):** the `useAnalytics` hook's `trackEvent` did a DIRECT `.from('analytics_events').insert({business_id, staff_id, event_name, meta})` ŌĆö a SECOND insert shape (`staff_id`/`meta`, not the RPC's `user_id`/`metadata`) that diverged from the RPC and would hit RLS. Routed through the canonical `record_analytics_event` RPC so there is ONE insert shape. The hook's public API (`track`/`trackImmediate`/`ANALYTICS_EVENTS`) is unchanged ŌĆö the 3 consumers (SecuritySettings, Automations, SSOSettings) are unaffected.

### Verification (P0.2)
tsc clean, vite build 0 warnings, vitest 94/94, schema-drift 0. Migration 111 idempotent (`ADD COLUMN IF NOT EXISTS`, `DROP FUNCTION IF EXISTS`, `CREATE OR REPLACE`). Needs live-DB application (same deploy-gate as the rest of 080ŌĆō110) to actually stop the 401, but the client now degrades cleanly until then.
...
**Gap A -- Onboarding flash NOT fully closed (Login.checkSession raced AuthContext).** Login.tsx's existing-session effect did its OWN single `.maybeSingle()` staff lookup with NO retry, racing AuthContext's 4-empty-read-retry-protected `fetchStaff`. Right after an OAuth callback redirect (or any case where the auth token/RLS context isn't ready on the first tick), Login's single read returned empty ŌåÆ navigated to `/onboarding` ŌĆö re-introducing the onboarding-flash bug AuthContext was hardened against (Session 12). Fix: Login.checkSession now does ONLY the MFA-challenge read (Login-specific UI) and defers the `/app` vs `/onboarding` decision to AuthContext's `staff`/`staffChecked` state via a new redirect effect that fires only when `staffChecked` is true. No more racy independent staff lookup.
...
1. Full login ŌåÆ session restoration ŌåÆ onboarding state: `getSession`ŌåÆ`setSession`ŌåÆ`fetchStaff`ŌåÆ`RequireAuth` checks `session+staffChecked+staff.business_id`; `OnboardingGate` mirrors it. Two independent gates converge on `business_id` as the authoritative "onboarded" signal (not the stale `onboarding_completed` flag).
2. Already-onboarded user never sent back: RequireAuth redirects to `/onboarding` ONLY if `!staff || !staff.business_id`; fetchStaff retries 4├Ś on empty; OnboardingGate + Onboarding.tsx both redirect to `/app` if `staff.business_id`; Onboarding handles the "already belongs to a business" RPC error via `refreshStaff`ŌåÆ`/app`.
...
4. Logout ŌåÆ login restores correct state: `signOut` clears session+staff+staffChecked, bumps `fetchIdRef`+`authGenerationRef` (stale requests discarded), clears module/experience/mfa caches. Re-login re-fetches staff for the new userId.
5. Expired sessions don't loop: `SIGNED_OUT` handler clears state + `staffChecked=true`; `TOKEN_REFRESHED` with null session ŌåÆ `setSession(null)` ŌåÆ RequireAuth ŌåÆ `/login`. No loop.
6. Service-worker cache: closed in R1.2 (`/sw.js` max-age=0, `updatefound`ŌåÆreload, version-purge).
7. `create_business_and_owner` anonymous-RPC: handled gracefully client-side (Onboarding + AuthCallback detect PGRST202 ŌåÆ appropriate fallback); genuinely blocked on live-DB migration application (same deploy-gate as #1/#2).
...
Audited the Dashboard against the user's 8-item P0.4 checklist. 4 were already done (workspace selection, no irrelevant KPIs, quick actions, active modules ŌĆö all via `useExperienceContext.isToolActive` from R2.8). 4 real gaps closed:

**#3 Capability-driven sections (partial ŌåÆ complete).** The "Business pulse" card showed a fabricated sparkline even when `primaryMetric.value === 0` (a user with no revenue/pipeline/projects saw a fake bar chart, not an honest state). Now shows an honest "No {metric} data yet. This will fill in as you use this tool." empty state when the value is 0 ŌĆö no fabricated chart.

**#4 Contextual Attention (task-only ŌåÆ aggregated).** The "What needs you" card only ever showed overdue TASKS. A finance user cares about overdue invoices; an inventory user about low stock; a CRM user about stale deals. Now aggregates every signal the user's active tools surface: overdue tasks (red) + overdue invoices (red, finance) + low-stock products (amber, inventory) + stale deals >14d (amber, CRM), each linking to the right page. The "Needs attention" KPI now counts the TOTAL across all signals (not just overdue tasks), so a finance-only user sees their overdue invoices reflected. New stats fetched: `overdueInvoices` (unpaid, non-draft, >30d old), `staleDeals` (open, >14d ŌĆö the intelligence threshold).

**#6 Role adaptation (none ŌåÆ role-aware Focus mode).** "My Focus" mode was a generic task list. Now role-aware: owner/admin ŌåÆ "Owner's view" (revenue/oversee-everything hint), manager/team_lead ŌåÆ "Manager's view" (active projects/team blockers), staff ŌåÆ "My work" (own tasks/deadlines). The pulse card's primary metric follows the role in focus mode (role-aware metric wins over the tool-driven default). The "Next actions" section shows the role label + hint in focus mode.

**#7 Company-size complexity (one card ŌåÆ structural).** `isSolo` previously only hid the People card. Now: solo businesses (a) don't see the mode tabs (no "operations" distinction ŌĆö a one-person business has no teams/departments to run operations across), and (b) don't see the Activity card (a one-person business has little cross-team activity noise; an empty "No recent activity" state is clutter, not value). Mid/enterprise keep the full layout. `isSolo` now derived from `complexity === 'solo'` (the authoritative progressive-complexity signal) instead of `companySize <= 1`.
...
**#4 Explain what each selected capability does.** The `TOOLS` array had only `key`/`label`/`category` ŌĆö no description. A new user picking "Time Tracking" or "Approvals" saw just the label with no idea what they were choosing. Added a `description` field to all 30 tools (plain-language summary of what each does). Onboarding's tool step + WorkspaceSettings now show the description under the label. RoleSettings (the third TOOLS consumer) uses `tool.label` only ŌĆö unaffected, but the description is available if needed later.

**#5 Prevent overwhelming users with the full module catalog.** WorkspaceSettings showed all curatable tools in one flat 20+ item grid ŌĆö overwhelming. Now groups tools by category (Core / Sell / Money / People / Work & Operations / Marketing / Support / Insights / Settings), ordered, each as a labeled section. The page is scannable instead of a wall of toggles. Onboarding already curates to a 16-tool SELECTABLE_TOOLS subset (good) ŌĆö unchanged.

**Checklist items verified done (no change):**
1. Feature selection in onboarding ŌĆö step 4 with industry-default seeding + `user_workspace_selections` upsert.
2. Revise later ŌĆö WorkspaceSettings at `/app/settings/workspace` with toggle + reset.
3. Separate selected from authorized ŌĆö `isToolAuthorized` gates; locked tools shown as "Not in your plan"; selection is a removal filter only (can never grant access).
6. Meaningful to role ŌĆö industry defaults seed the baseline; invited staff inherit business entitlements + role permissions (don't go through onboarding).

### Verification (P1.8)
tsc clean, vite build 0 warnings, vitest 94/94. Files: `src/lib/useToolAccess.ts` (added `description` to each TOOLS entry ŌĆö additive field, no type/consumer breakage since `ToolKey` derives from `.key` only), `src/pages/WorkspaceSettings.tsx` (group by category + show description), `src/pages/Onboarding.tsx` (show description in tool step). No migration/RPC/dependency changes.

### P1.9 -- WebsiteŌåÆonboardingŌåÆapp UX continuity (2 of 8 checklist gaps closed)
Audited the 3 surfaces (LandingEnhanced, Onboarding, app Shell/Dashboard) against the 8-item #9 checklist. 6 were already sound: same value-prop thread ("what needs your attention"/"what matters" runs LandingŌåÆDashboard); information hierarchy consistent (Landing preview "Good morning, David / 3 things need you" Ōåö Dashboard "Good morning, {name} / Here's what matters"); onboarding already leads into a personalized workspace (tool-selection step 4); Dashboard is adaptive not generic (P0.4); app Shell uses `--av-*` tokens consistently on mobile+desktop (no raw-color divergence). 2 real gaps closed:

**#2 Same visual language (the jarring jump ŌĆö CLOSED).** Onboarding used raw Tailwind `bg-blue-600` (#2563EB) + `border-black` + `text-black` ŌĆö a harsh, different palette from Landing's refined `BRAND` object (primary `#155BB4`, gradient logo, soft `#E4E8EF` borders, `#202124`/`#5F6368` text, `#F7F9FC` surface). The logo differed too (solid blue square vs gradient). Added a `BRAND` constant to Onboarding matching Landing's exactly, and replaced all raw colors: gradient logo, `#155BB4` primary (buttons/active step/selected ring), `#157342` success (completed steps), `#E4E8EF` soft borders (no more `border-black`), `#202124`/`#5F6368`/`#9AA0A6` text (no more `text-black`), `#F7F9FC` surface. The public site ŌåÆ onboarding ŌåÆ app now share ONE visual language.

**#5 Website promise reflected in onboarding (CLOSED).** Onboarding's sidebar was a generic "Welcome to Avenize / Let's set up your business" with no callback to the landing's promise. Added a value-prop line under the header: "Your whole business, connected. We'll set up what matters to you ŌĆö you can change anything later." ŌĆö echoing the landing's "one connected system / shows each person what matters to them" so onboarding doesn't feel like a generic setup wizard detached from what brought the user in.

### Verification (P1.9)
tsc clean, vite build 0 warnings, vitest 94/94. Single file changed: `src/pages/Onboarding.tsx`. No migration/RPC/dependency changes ŌĆö purely visual-language + copy alignment to the existing LandingEnhanced BRAND tokens.

### P1.10 -- Mobile/desktop design-system consistency (primary-color unification ŌĆö CLOSED)
Audited #10 (mobile+desktop same design system). Found a **three-way primary-color drift**: app chrome (`--av-primary` = `#4285F4`), Landing + Onboarding (`BRAND.primary` = `#155BB4`, darkened in Session 18 for WCAG-AA), and mobile (`colors.primary` = `#4285F4`). The web Landing/Onboarding moved to the darker accessible `#155BB4` but the app chrome (65 files consuming `var(--av-primary)`) and the mobile RN app were left on the lighter `#4285F4` (~3.5:1 on white, fails AA for small text). So the app a user landed in after onboarding was a visibly different blue than the onboarding they just completed ŌĆö the exact #10 gap.

**Closed via single-source-of-truth token change (not 65 file edits):**
- `src/styles/avenize-brand.css`: `--av-primary` `#4285F4`ŌåÆ`#155BB4`, hover `#3367D6`ŌåÆ`#1247A0`, active `#2A5DB0`ŌåÆ`#0F3B86`, soft/subtle rgba(66,133,244)ŌåÆrgba(21,91,180). Also `--av-gradient` aligned to the Landing BRAND gradient (`#155BB4ŌåÆ#4285F4ŌåÆ#34A853`). All 65 consuming files inherit the new primary automatically ŌĆö that's the power of CSS variables.
- `src/index.css`: `--avenize-gradient-start`/`--avenize-sales` `#4285F4`ŌåÆ`#155BB4`; gradient-mid/end aligned; the radial-gradient glow (line 126) fixed from `var(--av-primary)ŌåÆrgba(66,133,244,0.18)` (a two-blue mismatch once primary changed) to `rgba(21,91,180,0.18)`.
- `mobile/src/theme/index.ts`: `colors.primary` family `#4285F4`ŌåÆ`#155BB4` to match web. Mobile RN app now shares the same primary.

**Why this is safe:** `#155BB4` is WCAG-AA (6.3:1 on white, Session 18-verified) ŌĆö darkening is an accessibility improvement, not just consistency. White-on-`#155BB4` is MORE readable than white-on-`#4285F4`. The visual-regression test (`tests/ux/visual-regression.spec.ts`) has no committed baselines (`tests/ux/baselines/` doesn't exist) so the `if (fs.existsSync(baselinePath))` comparison branch never runs ŌĆö and all UX steps are `continue-on-error: true`. Semantic tokens (`--av-info` `#4285F4`, accentSales) left intentionally distinct (they're separate semantic colors, not the brand primary).
...
Audited #12 against its 8-item checklist. The external-market-data variance (items 1-external, 4, 5, 6, 7) requires sourced benchmark data ŌĆö `market_benchmarks` + `market_intelligence` RPC (063) exist but are empty; fabricating external numbers would violate ┬¦22 (anti-hallucination). Flagged as the "eventually" part the directive itself names. The buildable, honest slice ŌĆö item 3 ("what businesses say they need vs what they actually use") + item 1-internal ("which sectors use what") ŌĆö was closed using EXISTING tables:

**Migration `20260101000008_said_vs_used_reality_gap.sql`:**
- `said_vs_used(p_business_id)` (per-business, authenticated): compares `user_workspace_selections.selected_tools` (what they said they need at onboarding) against `usage_events` touches (what they actually use, 30d window matching `usage_module_adoption`). Labels: `selected_unused` (selected but never touched ŌĆö the headline waste gap), `used_unselected` (used but never selected ŌĆö a hidden need), `adopted` (3+ distinct staff), `trying` (1-2), `untouched`. Follows the exact precedent of `usage_module_adoption` (SECURITY DEFINER + business_id filter + granted to authenticated).
- `sector_module_usage()` (builder-only, service role): aggregates across all businesses by `businesses.industry` ŌĆö for each (industry, module_key): businesses_selecting, businesses_using, adoption_rate. Serves item 1 (sectors using what) + item 2 (over/under-performing by sector). REVOKED from anon/authenticated (cross-business aggregate ŌĆö builder-facing, matches `usage_cross_business_adoption` precedent). `adoption_rate` is NULL (not 0%) when a sector selected nothing ŌĆö honest.

**Client + UI:** `fetchSaidVsUsed()` wrapper in businessOS.ts (best-effort, non-blocking per ┬¦24 ŌĆö stays empty if migration not deployed). RealityGap.tsx gained an "Auto-detected: said vs used" section surfacing the two gap types (selected_unused as warn, used_unselected as info) with event/staff counts + INFERENCE tag ŌĆö turning the previously-manual-only page into one that surfaces AUTOMATIC gaps from real telemetry.
...
### What remains for #12 (the "eventually" external-data part ŌĆö blocked on data sourcing, not code)
...
### Verification (P1.12)
tsc clean, vite build 0 warnings, vitest 100/100 (was 94, +6 new). Schema drift 0. Files: `supabase/migrations/20260101000008_said_vs_used_reality_gap.sql`, `src/lib/businessOS.ts` (wrapper + type), `src/pages/RealityGap.tsx` (auto-detected section), `tests/frontend/lib/saidVsUsed.test.ts`. No new dependencies, no external APIs ŌĆö all deterministic SQL over real tables.

### P1.14 -- Platform self-instrumentation (the PRD #14 metric capture ŌĆö CLOSED)
...
- 5 per-business RPCs (authenticated): `onboarding_funnel` (completed_at + steps_reached + duration from the onboarding_complete event), `workflow_funnel` (started/completed/abandoned/completion_rate per workflow), `feature_activation` (first-active, distinct active days, reuse_label: reused/returning/activated/view_only), `ignored_automations` (derives from automations + automation_runs ŌĆö created-but-never-triggered), `quick_turnoff` (tool_selectŌåÆtool_deselect within 7d = "modules switched off quickly").
- 1 builder-only RPC: `onboarding_conversion` (cross-business ŌĆö total_authenticated / total_completed / total_abandoned / conversion_rate / median_steps / avg_duration). **ABANDONMENT IS A FACT, not an inference** ŌĆö derived from `auth.users` LEFT JOIN `staff` (authenticated user with no staff record = abandoned), stronger than tab-close guessing (┬¦22). REVOKED from anon/authenticated (cross-tenant auth data ŌĆö service role only).

**Why abandonment as auth-gap, not event-gap:** `usage_events.business_id` is NOT NULL FKŌåÆbusinesses; during onboarding the business doesn't exist until `create_business_and_owner` succeeds, so step events mid-flow can't be logged against a business_id. The honest, schema-correct approach: emit ONE `onboarding_complete` event (with steps_reached + duration context) AFTER success; derive abandonment from the auth.usersŌåÆstaff gap server-side. This captures completion timing/steps (FACT from the event) + abandonment (FACT from the gap) without the tab-close inference problem.

**Client instrumentation (fire-and-forget, never blocks UX):**
- `logUsageEvent()` helper in `useUsageTracking.ts` ŌĆö structured events with context.
...
- **useWorkspaceSelection.ts `toggleTool`**: emits `tool_select`/`tool_deselect` (context: tool) ŌĆö the single source of truth, fires for both onboarding + settings-page toggles. Pairs with `quick_turnoff`.
- **Quotes.tsx**: emits `workflow_start` at createQuote, `workflow_complete` at sendQuote (milestone:sent) + convertToInvoice (milestone:converted) ŌĆö the quote workflow lifecycle (draftŌåÆsentŌåÆconverted).

**Test:** `tests/frontend/lib/selfInstrumentation.test.ts` (7 tests) locks the reuse_label contract (reused/returning/activated/view_only) + the onboarding-abandonment-is-a-FACT contract (hasStaffŌåÆcompleted, !hasStaffŌåÆabandoned).
...
### #18 ŌĆö Owner-only business intelligence (the private intelligence layer ŌĆö CLOSED)
...
**CRITICAL security fix found while building #18:** the 5 per-business RPCs from #14 (`feature_activation`, `quick_turnoff`, `workflow_funnel`, `ignored_automations`, `onboarding_funnel`) were SECURITY DEFINER + granted to authenticated but took a `p_business_id` param WITHOUT verifying the caller belongs to that business. SECURITY DEFINER bypasses RLS, so any authenticated user could pass another business's UUID and read its analytics ŌĆö a cross-tenant leak I introduced in #14. `onboarding_funnel(p_business_id DEFAULT NULL)` was worse: NULL meant "all businesses" (a builder query exposed to any customer). **Migration `20260101000010` re-declares all 5 with an ownership guard** (`EXISTS (SELECT 1 FROM get_current_staff() cs WHERE cs.business_id = p_business_id)`); `onboarding_funnel` NULL now resolves to the caller's own business via `COALESCE`. Empty result if not a member (safe ŌĆö no error, no leak). The CI migration-apply gate did NOT catch this (it tests SQL validity, not authorization) ŌĆö found by the #18 audit forcing me to think about the authorization layer properly. Lesson: SECURITY DEFINER functions that take a business_id param MUST gate on membership; RLS does not protect them.

**The owner_intelligence aggregator (checklist #18):**
- `owner_intelligence(p_business_id)` JSONB RPC in migration `20260101000010`. PL/pgSQL (not sql) so it can branch on the gate. Verifies the caller's role is `owner`/`admin` AND a member of the business via `get_current_staff()` (the real boundary ŌĆö defense-in-depth + checklist #18 "Owner/admin permission"). Returns empty JSONB payload if not owner/admin (safe, no leak). Returns feature activation + reuse, quick-turnoff, ignored automations, workflow funnel, and onboarding completion in ONE structured call (not 5 round-trips). `data_scope: 'operational_and_usage_only'` field declares the #21 boundary structurally.
- **#21 boundary (verified + documented):** the aggregator reads ONLY `usage_events` + `automations` + `automation_runs` ŌĆö operational/usage data. It NEVER references `legal_cases`, `legal_contracts`, `legal_obligations`, disciplinary records, `payroll_records`/`salary_history`, board finance, or litigation. Excluded BY CONSTRUCTION (the function body never touches those tables), not by a runtime toggle. The page surfaces this boundary as a visible banner.

**Three-layer defense-in-depth (checklist #28 "No client-side security decisions"):**
1. **Client UX gate** ŌĆö `Shell.itemVisible` `ownerOnly` flag + `OwnerIntelligence.useIsOwnerAdmin()` hide the nav item + page from non-owners/admins. UX ONLY.
2. **RPC server gate** ŌĆö `owner_intelligence` verifies role + membership via `get_current_staff()`. THIS is the security control (SECURITY DEFINER bypasses RLS). A tampered client / direct URL / staff member gets an empty payload.
3. **RLS** ŌĆö `usage_events` is business-scoped (Session 9).

**Builder-only cross-business analytics (NOT exposed to customers):** `onboarding_conversion`, `sector_module_usage`, `usage_cross_business_adoption` stay REVOKED from authenticated (service-role only). These serve the Avenize builder/board dashboard (checklist #19/#34), not any customer ŌĆö a customer never sees another business's data. The customer-facing layer is `owner_intelligence` (own business only).

**Client layer:**
- `fetchOwnerIntelligence()` wrapper in businessOS.ts (best-effort ┬¦24 ŌĆö returns null when RPC not deployed).
...
### #16/#17 ŌĆö Sector intelligence + behavior-driven recommendations (CLOSED)
Checklist #16 (market intelligence / reality-gap) + #17 (behavior-driven recommendations).

**Honest scope split (┬¦22 anti-fabrication):**
- BUILDABLE now: the INTERNAL sector benchmark ŌĆö a business's own module-adoption vs its sector's ANONYMIZED aggregate (count/avg only, never individual businesses). First-party data only.
- BLOCKED on sourced data (not fabricated): #16 items 4-7 (emerging sector behavior, product-market gaps, new-feature opportunities, industry positioning) need real external market data ŌĆö a Tavily/sector-report integration. Fabricating these would violate ┬¦22. Documented as blocked, same as #12.
...
**Behavior-driven recommendation rules (#17) ŌĆö `run_behavior_recommendation_rules(p_business_id)`:**
- A SEPARATE function (NOT a re-declaration of `run_recommendation_rules` ŌĆö that would drop the 8 original rules from 091). The cron fan-out calls BOTH.
- **USAGE-001** (info): modules selected-but-unused in 30 days. Guard: `selection_completed = true`.
- **USAGE-002** (warning): workflow abandonment > 50%. Guard: Ōēź 3 starts.
- **SECTOR-001** (info): sector-popular module not enabled. Guards: sector sample Ōēź 5 AND adoption Ōēź 50%.
- Each: specific, small-data-guarded (┬¦21), idempotent (`has_open_recommendation`), best-effort (EXCEPTION ŌåÆ 0). Wired into the cron: `run_all_recommendation_rules` (092) re-declared to call both per business.

**Client layer:** `fetchSectorBenchmark()` + `runBehaviorRecommendationRules()` wrappers (best-effort ┬¦24). OwnerIntelligence.tsx "Sector Benchmark" section (anonymized, sample-size labels, small-sample warning). Recommendations surface in the existing Executive Cockpit `RecommendationsCard` (no new UI ŌĆö rules write to `claims`, the existing feed reads them).
...
### #19/#34 ŌĆö Builder / board dashboard (platform-operator surface ŌĆö CLOSED)
Checklist #19 (sector/module analytics + market reality-gap) + #34 (board dashboard). This is the **platform-operator** surface ŌĆö distinct from the per-business owner intelligence (#18). The Avenize builder uses it for sprint/product decisions ("which of the 61 modules actually get touched" ŌĆö empirically, independent of entitlements).

**The platform-admin distinction (critical, NOT a business role):** there was NO existing platform-builder concept (`staff.role` is owner|admin|manager|team_lead|staff ŌĆö all business roles). A business owner is NOT a platform admin. Migration `20260101000012` adds a `platform_admins` email allowlist table (RLS denies ALL client access ŌĆö service role only manages it). `is_platform_admin()` checks `auth.uid()`'s email against the allowlist. `builder_dashboard()` is gated by this ŌĆö a business owner gets `{authorized: false}` (empty, safe, no leak). The builder is a real Avenize operator (their auth email is added to the allowlist by the service role), not a business user.

**`builder_dashboard()` JSONB RPC (migration 20260101000012):** SECURITY DEFINER so it can call the 3 service-role-only cross-business RPCs (`sector_module_usage`, `onboarding_conversion`, `usage_cross_business_adoption`) ŌĆö which stay REVOKED from authenticated. This RPC is the ONLY authenticated-callable aggregator. Returns one JSONB payload: onboarding conversion (total_authenticated/completed/abandoned, conversion_rate, median_steps, avg_duration), cross-business module adoption (module_key, businesses_touching count, total_events count), sector├Śmodule adoption (industry, module_key, selecting/using counts, adoption_rate).

**#21 boundary (aggregate only, verified + documented):** the payload contains ONLY counts/rates/averages ŌĆö NEVER business names, owner emails, customer names, invoice amounts, legal/disciplinary/payroll data. The RPC reads only `usage_events` + `user_workspace_selections` + `businesses.industry`. The page surfaces this as a visible banner. The underlying cross-business RPCs stay service-role-only; direct client calls are denied.

**Defense-in-depth:** (1) client route behind `RequireAuth` (needs a session), (2) `builder_dashboard` RPC verifies `is_platform_admin()` via `auth.uid()` (the real boundary), (3) `platform_admins` RLS denies client writes (only service role can grant platform access ŌĆö prevents a business user self-elevating).

**Route:** `/builder` (top-level, NOT under `/app` ŌĆö it's not a business surface). The RPC gate is the authority; a business user typing `/builder` gets the "unauthorized" screen, not the analytics.

**Client layer:** `fetchBuilderDashboard()` wrapper (best-effort ┬¦24). BuilderDashboard.tsx page: onboarding conversion stats, platform-wide module adoption bars, sector├Śmodule table (selecting vs using vs adoption%, low-adoption flagged amber), and an honest "what this cannot tell you" section (external market variance blocked-on-sourced-data, ┬¦22).
...
- **`run_due_automations()` scheduled executor:** finds enabled automations with `trigger_type = 'scheduled'` whose cron window is due (idempotent ŌĆö tracks `last_run_at` in `trigger_config` JSONB, 55-minute floor guards against double-firing within the hourly window). Best-effort per automation (EXCEPTION ŌåÆ log + skip, never aborts the batch). Calls the existing `execute_automation_action`.
...
- **`automation_health` surfaced** on the OwnerIntelligence page (success rate bar, recent runs list, never-run flag) ŌĆö best-effort, non-blocking.
...
### #extensibility ŌĆö API key gateway + plaintext-storage fix (CLOSED)
...
- `platform_incident_investigations` (AUDIT TRAIL for tenant drill-down ŌĆö drilling into a tenant's data is an explicit logged action, never silent; the scope's critical privacy boundary)
- `log_platform_error` (authenticated, fire-and-forget, swallow-on-failure ŌĆö never breaks a user's request path, idempotent)
- `record_integration_check` (service-role only; the scheduled health checker writes here)
- `evaluate_platform_alerts()` (thresholdŌåÆincident automation, idempotent ŌĆö no duplicate incidents, auto-resolves when condition clears, best-effort per rule, pg_cron every 3 min guarded like 051)
- `platform_ops()` (the aggregator ŌĆö `is_platform_admin()`-gated, ONE call returns the live-status-strip payload: per-system traffic-lights, recent errors, integration health, open incidents; **aggregate + structural only ŌĆö no PII, no financials**)
- `resolve_platform_error` / `update_platform_incident` / `investigate_business_incident` (platform-admin-gated mutation RPCs)

**Client layer:**
- `PlatformOpsDashboard.tsx` at `/platform-ops` (top-level, NOT `/app` ŌĆö not a business surface; mirrors `/builder`). Behind `RequireAuth` + the `is_platform_admin()` gate (non-admins get the "restricted" screen). Live status strip (traffic-light per major system), realtime error feed (filterable by severity), integration health panel, incident log with postmortem. **Realtime** subscription to `platform_error_events` + `platform_incidents` inserts (per-mount channel name, no polling ŌĆö the NotificationBell pattern).
...
- `businessOS.ts`: `fetchPlatformOps`/`logPlatformError`/`resolvePlatformError`/`updatePlatformIncident`/`investigateBusinessIncident` wrappers (best-effort, non-blocking ŌĆö degrade gracefully if migration not deployed, per ┬¦24).

**Tests (24):** `tests/frontend/lib/platformOps.test.ts` locks: the platform-admin gate (NOT a business role), the aggregate-only privacy boundary (no PII/financials), the async/non-blocking ingest contract (swallow-on-failure), the thresholdŌåÆincident idempotency (no dupes, auto-resolve on clear), the tunable-threshold contract (data not code), the audit-logged tenant drill-down boundary, integration failure-streak reset. vitest 164 ŌåÆ 188.

### Verification
tsc clean, vite build 0 warnings, vitest 188/188, schema-drift 0. CI green on push (Type Check, Unit Tests, Schema Drift, Migrations Apply Clean, Build all success). Commits: 874fd5a, 24068fc, ff8995c, a7663dd, 7b785c8 ŌĆö all pushed to main.

### Deploy status
- Vercel production: deploying via the main-push workflow.
- ŌÜĀ’ĖÅ STILL needs live DB: pending migrations must be applied to Supabase (project `kgsgqvatyleetyquffya`). All idempotent. Frontend degrades gracefully until then (ops dashboard shows "couldn't load ŌĆö migration may not be applied yet" with retry; the platform-admin gate returns `authorized:false` for everyone until `is_platform_admin()` exists). No new external dependencies; no WhatsApp/Meta.
...
## Session 24 (2026-08-18): The Avenize Business Brain ŌĆö State + Diagnosis + Next Best Action + Value Ledger

Triggered by the top-20 priority directive: turn the many features already built into "one coherent, intelligent, resilient organism" ŌĆö a Business Intelligence layer where every module is connected by a Brain that reasons across data. Verified-before-build (the audit protocol): the directive is NOT "add more features" ŌĆö most named capabilities already exist as infrastructure. The gap is the layer that reasons ACROSS modules: What is happening? Why? What should I do? Did it work? How much value did it create?

### Verified FIRST (the infrastructure is substantial ŌĆö don't rebuild it)
...
- **Recommendation + Outcome loop** (claims/088/091): claims has the FULL lifecycle (issuedŌåÆacknowledgedŌåÆacceptedŌåÆrejectedŌåÆactedŌåÆoutcome_recordedŌåÆsupersededŌåÆexpired) with expected_impact + actual_impact JSONB + linked_action_id + rule_id + severity + action_type + owner_id. open_recommendations (TABLE return). recommendation_effectiveness (TABLE: rule_id, issued, accepted, rejected, acted, outcome_recorded, success_count, avg_actual, avg_expected). 8 deterministic rules (091) + 3 behavior rules (20260101000011). The recommendationŌåÆactionŌåÆoutcomeŌåÆvalue loop ALREADY EXISTS ŌĆö item H (Value Ledger) is an aggregation VIEW over claims, not net-new.
...
- **Business Risks** (095): the general risk register (probability ├Ś impact = risk_score).
...
### The GENUINE gaps (the three Brain pillars ŌĆö all built this session)
1. **Business State Engine (D, #4)** ŌĆö classify_business_state: 12-state priority-ordered classifier (growing/stable/scaling/stressed/recovering/at_risk/cash_constrained/sales_constrained/capacity_constrained/operationally_constrained/opportunity_rich/insufficient_data) from health scores + metric MoM trends. at_risk (<40) before cash/sales constraints before stressed (40-55) before recovering (56-69 + rising) before growing/scaling (>=70 + strong growth) before opportunity_rich (>=70 + building pipeline) before stable (>=70). The state INFLUENCES what Avenize shows.
2. **Diagnosis Engine (E, #6) ŌĆö the differentiator** ŌĆö diagnose_business: instead of "Revenue is down 8%", reasons "Revenue is down 8% because conversion dropped 11% ŌĆö Ōé”X monthly exposure." Each diagnosis = SYMPTOM (significant metric change ŌĆö FACT) + CAUSE (correlated metric change ŌĆö INFERENCE) + IMPACT (Ōé” from real numbers). Causal relationships are DECLARED as rules (diagnosis_rules table ŌĆö tunable by Avenize operators, the scope's "thresholds are a business decision"), NOT inferred by an LLM. 6 rules seeded (revenue-conversion, revenue-pipeline, cash-overdue, cash-expenses, profit-margin-erosion, ops-task-overload).
3. **Next Best Action Engine (F, #7)** ŌĆö next_best_action: instead of overwhelming with a list, surfaces the SINGLE most valuable thing to do now. Scores open recommendations by financial_impact (log scale so Ōé”10M doesn't drown Ōé”50k) ├Ś urgency (severity) ├Ś probability_of_success (from the effectiveness loop, 088) / effort (action_type heuristic) + business-state relevance bonus. Returns the top action with owner + due date + the scoring reasoning.
4. **Business Value Ledger (H, #9)** ŌĆö business_value_ledger: aggregates the existing recommendationŌåÆactionŌåÆoutcome loop into "Avenize helped recover Ōé”X / saved Ōé”X / generated Ōé”X / identified Ōé”X." ONLY from REAL recorded outcomes (status=outcome_recorded + actual_impact.amount ŌĆö ┬¦22, never fabricated). When no outcomes: total=0 + honest note. Categorizes by rule prefix (FIN-AR/CF=recovered, SAL/CUST=generated, INV/OPS/DQ=saved). The retention mechanism.
5. **business_brain(business_id)** ŌĆö ONE aggregator returning State + Pulse + Diagnoses + Next Best Action + Value Ledger ŌĆö the intelligence-first dashboard renders in a single round-trip. Membership-guarded (get_current_staff).

### ExecutiveCockpit ŌĆö the Brain is the intelligence-first surface
...
- DiagnosisCard: "What we found ŌĆö and why" with FACT/INFERENCE tags + Ōé” exposure.
...
2. `v_metrics := current_metrics(...)` assigned a TABLE-valued return to a JSONB variable ŌĆö must use `SELECT COALESCE(jsonb_agg(to_jsonb(t)), '[]'::jsonb) INTO v_metrics FROM current_metrics(...) AS t` (in BOTH classify_business_state and diagnose_business).
3. `v_recs := open_recommendations(...)` ŌĆö same TABLE-to-JSONB coercion needed in next_best_action.
4. `recommendation_effectiveness` returns TABLE columns (rule_id, success_count, outcome_recorded) ŌĆö NOT a JSONB 'outcomes' array. The NBA probability calc must `SELECT 1.0 * e.success_count / NULLIF(e.outcome_recorded, 0) FROM recommendation_effectiveness(...) e`.
5. `current_metrics` column is `name`, not `display_name`.
6. `ClaimTag` component uses `type` prop, not `level`.
7. Diagnosis headline string construction had mismatched quotes ŌĆö rewrote the block cleanly.

### Tests (+36, 291 total)
- businessState.test.ts (12): the classifier priority chain ŌĆö at_risk beats cash_constrained beats stressed; cash/sales/capacity/operational constraints; recovering vs growing vs scaling vs opportunity_rich vs stable; insufficient_data when no health score.
...
- nextBestAction.test.ts (7): single highest-scoring action; null when empty; state relevance bonus; log-scale impact (Ōé”10M vs Ōé”50k, equal effort); probability (0% success halves score); effort (reminder > PO); critical > info.
...
- #8 (the Brain connecting modules): business_brain aggregator ŌĆö the modules are now connected by a reasoning layer.
...
### False-success closure (commits 709895d, 31923c6) ŌĆö ┬¦AC/┬¦76
...
regardless of whether the write persisted ŌĆö a failed write looked like
success. Fixed: Announcements (7), BusinessInfrastructure (7), Organization
(3), HumanResources (5), FinanceCenter (5), LeaveManagement (2),
ExpenseClaims (4 ŌĆö incl. 2 status-updates that the script's closing-brace
detection missed; fixed manually), Meetings (2 ŌĆö sendInvites/sendReminder now
...
### .single() first-run crashes closed (commit a28ab02) ŌĆö ┬¦R1 Session-10 class
Audited ~30 `.single()` lookups. Most are post-insert `.select().single()`
(CORRECT ŌĆö insert returns one row) or by-id lookups in try/catch (work). Two
genuine first-run breakers fixed: Services.tsx `businesses.slug` lookup used
`.single().then(...)` ŌĆö on a business with no slug, .single() rejects ŌåÆ
unhandled rejection + bookingLink never set ŌåÆ .maybeSingle(). PDFGenerator.ts
`business_branding` lookup used `.single()` ŌĆö a fresh business with no
branding row threw ŌåÆ PDF generation crashed ŌåÆ .maybeSingle() (the
`return data || {...}` fallback now actually receives null).

### Rate limiting wired into Login + Signup (commit a28ab02) ŌĆö ┬¦AD
`check_auth_rate_limit` + `log_security_event` (migration 999) were built but
had ZERO callers ŌĆö Login/Signup relied only on Supabase's built-in throttle.
- Migration 20260818290000: GRANT EXECUTE on the two pre-auth functions to
  ...
  authenticated). Least-privilege ŌĆö only these two, not the blanket.
- Login.tsx: calls check_auth_rate_limit(email,'login',5/300s/900s-lockout)
  ...
  + proceed) ŌĆö never blocks login because rate-limit infra errored.
- Signup.tsx: same (signup, 5/3600s/3600s).

### Business Brain degraded-engine flags surfaced (commit fe78aae) ŌĆö ┬¦N
The ┬¦N graceful-degradation layer (migration 20260818240000) isolates each
Brain sub-engine in its own EXCEPTION block ŌĆö a failed engine returns
...
unavailable ŌĆö the rest of your business is unaffected; this will refresh
automatically" notice. Added `degraded?: boolean` to the 4 interfaces
(type-safe). The "built but never wired" ┬¦N gap closed.

### Design-system completion (commits 824ea95, 63d2665) ŌĆö the av-* token directive
- 824ea95: public/auth flow (Signup/NotFound/AuthCallback) ŌĆö the
  signupŌåÆonboardingŌåÆapp journey now shares ONE visual language. 10 raw
  classes ŌåÆ tokens in Signup; LandingEnhanced left (intentional dark mock).
- 63d2665: SEMANTIC STATUS colors across 116 files / 839 class swaps. Status
  ...
  solid status bg (bg-{red,green,amber}-500/600) ŌåÆ av-{success,danger,
...
fully, still the old flow." Two real, user-facing gaps ŌĆö both about the ACTUAL
...
### Gap 1 ŌĆö The /app flow was STILL the old module-first experience (┬¦A #1-3)
The user's core complaint verified: landing on `/app` rendered `CompanyHome`
(My Work / Culture tabs) ŌĆö the attention page from Session 17 ŌĆö with the
...
intelligence-first") were NOT done ŌĆö a user's first screen was the old flow.
...
- **Business State card** ŌĆö "how is my business doing right now" ŌĆö the
  ...
  + the top reason + "See the full picture" ŌåÆ /app/cockpit.
- **Next Best Action card** ŌĆö "what should I do now" ŌĆö the `next_best_action`
  result with statement + expected impact (Ōé”) + "Take action" ŌåÆ cockpit, or
  the healthy "Nothing needs your attention right now" state.

The personal attention layer (approvals/tasks/messages) is preserved BELOW ŌĆö
...
- `business_brain` RPC is membership-guarded (any staff member, not owner-
  only) ŌåÆ the hero is safe for ALL users; a staff member sees the same
  business state as the owner.
- Best-effort (┬¦24): brain stays null if migration not deployed ŌåÆ hero simply
...
- Tokenized the CompanyHome BRAND object (raw #F8F9FA/#202124/#E8EAED ŌåÆ
  var(--av-surface-2/text/border)) so the home page resolves through the
  single --av-* source.

### Gap 2 ŌĆö Empty states were flat dead-end notices, not gamified (┬¦AC)
The user: "fix the empty states in the app with gamified instructions." Empty
states were flat "No X yet" + description + button ŌĆö a dead-end notice that
...
- `deriveMilestone()` auto-turns "No deals yet" ŌåÆ "Your first deal".
- Backward compatible: existing callers (no gamified fields) render exactly
  as before. No consumer broke.
- Tokenized the legacy raw colors (text-black/bg-white/bg-slate-700 ŌåÆ
...
DocumentsHub, LeaseManagement, MaintenanceRequests, ElectronicSignatures) ŌĆö
...
- Dashboard primaryMetric empty: "Building your [revenue] story" milestone +
  honest "this fills in as you use this tool ŌĆö your first entry starts the
  trend" (replaces the flat sparkline-gap + "No data yet").
- Dashboard + CompanyHome "all caught up": reframed to a positive milestone
  ("Inbox zero" / "a clear desk is progress") ŌĆö absence of work is framed as
...
- CRM deals + contacts inline empties: "Your first deal/contact" milestone +
  ...
  deal") ŌĆö gamifies the CRM onboarding moment.

Also tokenized the `EmptyStates.tsx` (plural) base component (bg-slate-700,
text-black, text-amber-800 ŌåÆ av-* tokens) ŌĆö the duplicate-pair from the
...
- **Intelligent notification priority ordering (┬¦Z "no notification spam").**
  NotificationBell ordered ONLY by created_at desc ŌĆö a brand-new
...
### Verification
tsc clean, vite build 0 warnings, vitest 368/368, schema-drift 0. No new
migration/RPC/dep changes ŌĆö the Brain was already built (20260818220000); this
...
ŌÜĀ’ĖÅ STILL needs live DB: pending migrations (incl. 20260818220000 business_brain)
...
(the hero returns null) ŌĆö no error, no broken shell.
...
### Architecture ŌĆö composition, not 8 separate homepages
- `src/lib/roleHomeConfig.ts` ŌĆö the centralized roleŌåÆconfig map. Each role
  (owner/admin, manager, team_lead, staff ŌĆö all 5 DB-valid roles) declares
  ...
  the home architecture. SECURITY: role personalization is UX ONLY ŌĆö
...
- `src/components/BusinessHomeCards.tsx` ŌĆö reusable intelligent card
  ...
  link. No fabricated metrics (┬¦22) ŌĆö every number from a real RPC
  (business_brain / current_metrics / current_business_health /
  open_recommendations / profitability_leakage / value_ledger).
- `src/pages/BusinessHome.tsx` ŌĆö the orchestrator. Fires all intelligence
  loads in parallel (best-effort, ┬¦24 ŌĆö one engine failing never collapses
  ...
  intelligence layer ŌĆö not removed, no longer primary.
...
- `avenize-brand.css`: glass tokens (`--av-glass-bg/blur/border`),
  ...
  people/intelligence`) ŌĆö one design system, mood by domain. All derive
...
- Subtle motion communicates life: ambient state glow (pulsing dot coloured
  by state ŌĆö Growing=green, Stressed=amber, At risk=red), Business Pulse
...
(Finance/Sales/Customers/Ops/People/Projects) as a connected glowing node ŌĆö
...
- Owner/Admin ŌåÆ whole-business: state, NBA, revenue, cash, profit, pulse +
  opportunities, risks, operations, value ledger, diagnoses.
- Manager ŌåÆ cross-functional execution: state, NBA, operations, people,
  pipeline, pulse.
- Team Lead ŌåÆ delivery: NBA, operations, people, pulse.
- Staff ŌåÆ personal work: NBA, operations (limited business visibility).

### Route + Shell
- `/app` index ŌåÆ `BusinessHome`. `CompanyHome` moved to `/app/community`
...
- Shell: added "Ask Avenize" primary entry point in the desktop top bar
  (gradient pill) + mobile (gradient Ask button) ŌĆö the intelligence entry
  point the brief named as important. Existing Quick Capture sidebar button
  preserved.

### Resilience (┬¦N)
If the Brain migration isn't deployed, cards degrade gracefully (honest "ŌĆö"
+ "building your picture"), hero falls back to greeting, My Work still
renders. One engine failing ŌåÆ degraded banner on that card, rest unaffected.
...
migration/RPC/dep changes ŌĆö pure frontend orchestration over existing
...
### Deploy status
Vercel production: deploying via main-push workflow.
ŌÜĀ’ĖÅ STILL needs live DB: pending migrations (incl. 20260818220000
...
degrades gracefully to greeting + My Work + honest "ŌĆö" cards ŌĆö no error,
no broken shell. The visual transformation (glass/gradient/Pulse) renders
immediately (pure CSS).

## Session 28 (2026-08-18): App-wide visual consistency ŌĆö atmospheric backdrop + glass chrome + premium cards
...
flat white cards ŌĆö a visible seam between the home and every other page.
This unifies the whole app to ONE visual language without rewriting 129
page files. 1 commit (cd2bb0e), pushed to main.

### Highest-leverage change ŌĆö Shell atmospheric backdrop (affects all pages)
- The Shell root + content wrapper now use `.av-backdrop` (the same
  ...
  uses). Every page now sits on the premium atmospheric backdrop ŌĆö the seam
...
- Sidebar, desktop top bar, mobile header, mobile bottom nav softened to
  ...
  chrome now matches the home's glass language ŌĆö one organism, not
...
- avenize-brand.css adds `.av-card` (solid elevated surface + float shadow
  + large radius ŌĆö for data tables/lists/forms where opacity would hurt
  readability) and `.av-glass-card` (translucent glass + blur ŌĆö for
...
- Dashboard `Card` ŌåÆ `.av-card`; replaced all 24 slate-* tokens with av-*
  (the flat white + raw-shadow intelligence cards now match the home).
- ExecutiveCockpit: 18 flat-white helper cards ŌåÆ `.av-card` (premium solid).
- Tasks, Approvals, StaffProfile, RoleSettings, SSOSettings: all slate-*
  tokens replaced with av-* (zero slate residue across app pages ŌĆö only
  Landing marketing retains its own palette, by design).

### Verification
tsc clean, vite build 0 warnings, vitest 368/368, schema-drift 0. Zero
slate residue in app pages. No migration/RPC/dep changes ŌĆö pure CSS token
...
renders immediately ŌĆö pure CSS, no migration needed. The Brain cards still
populate only once pending migrations are applied to the live Supabase
(same deploy-gate as prior sessions).

## Session 29 (2026-08-18): Function ├Ś Seniority ŌĆö Marketing/Sales/Finance/HR/Ops/Projects homes
...
Lead/Staff into Function ├Ś Seniority ├Ś Permission ├Ś Personal Work ├Ś
...
this extension ŌĆö so I extended, did not rebuild. 1 commit (0e2d8bb), pushed
to main.

### Architecture ŌĆö Function ├Ś Seniority
...
- `src/lib/functionHome.ts` ŌĆö `deriveFunction(jobTitle, department,
  activeTools)` ŌåÆ the 7 functions (general/marketing/sales/finance/hr/
  ...
  the function's priority cards + the whole-business pulse ŌĆö they oversee the
...
- `BusinessHomeCards.tsx`: 7 new pure card primitives
  ...
  LeaveBalanceCard, ProjectDeliveryCard, WorkloadCard) ŌĆö each takes a data
  prop, renders via the same GlassCard shell, FACT/INFERENCE confidence tags,
  honest "ŌĆö" empty states with gamified instructions. No fabricated metrics.
- `BusinessHome.tsx`: resolves fn ├Ś sen, fires the 7 function-specific loads
  in parallel (best-effort, non-blocking ŌĆö a missing/empty table degrades to
  "ŌĆö" honestly), passes the data through CardConfig ŌåÆ CardByKey. Adaptive hero
  eyebrow now reads "{Function} engine at a glance ┬Ę {Seniority}" for function
  homes; "Your business at a glance ┬Ę Executive" for general.

### The function homes
- Marketing ŌåÆ campaign performance, lead quality, pipeline contribution,
  pulse. Pulse sequence: CampaignsŌåÆReachŌåÆLeadsŌåÆQualifiedŌåÆOpportunitiesŌåÆ
  PipelineŌåÆRevenue.
- Sales ŌåÆ pipeline, revenue, customers, pulse. Pulse: LeadsŌåÆQualifiedŌåÆ
  OpportunitiesŌåÆProposalsŌåÆNegotiationsŌåÆWonŌåÆRevenue.
- Finance ŌåÆ cash, receivables, profit, pulse. Pulse: InvoicingŌåÆReceivablesŌåÆ
  CashŌåÆExpensesŌåÆPayablesŌåÆProfitŌåÆHealth.
- HR ŌåÆ people, attendance, leave, pulse. Pulse: HeadcountŌåÆHiringŌåÆAttendanceŌåÆ
  LeaveŌåÆPayrollŌåÆEngagementŌåÆRetention.
- Operations ŌåÆ operations, workload, pulse. Pulse: SuppliersŌåÆInventoryŌåÆ
  OrdersŌåÆFulfillmentŌåÆCapacityŌåÆThroughputŌåÆCost.
- Projects ŌåÆ project delivery, workload, pulse. Pulse: BacklogŌåÆActiveŌåÆ
  MilestonesŌåÆWorkloadŌåÆBlockedŌåÆDeliveryŌåÆMargin.

### SECURITY (the critical invariant)
Function personalization is UX ONLY ŌĆö it emphasizes cards, never grants
...
RLS denies ŌĆö `deriveFunction` only changes what is EMPHASIZED, never what is
EXPOSED. The seniority axis (from the DB role) and the existing module gate
(RequireModule) are unchanged. The role model didn't change ŌĆö the same 5
...
### Resilience (┬¦24)
If any function table is empty/missing, that card renders an honest "ŌĆö" +
...
collapses the home ŌĆö the Brain RPC cards populate independently. Every load
...
contracts), schema-drift 0. No migration/RPC/dep changes ŌĆö pure frontend
...
cards render immediately against real tables (no migration needed ŌĆö the
...
## Session 23b (2026-08-18): Enterprise onboarding hardening ŌĆö migration idempotency + alert->toast
...
### Migration idempotency + apply-cleanliness (commit 19c9cc1) ŌĆö the highest-value work
...
overloads):
1. board_members invites_role_check constraint drop failed ŌĆö Postgres
...
### Subsidiary data isolation ŌĆö verified SOUND (not a gap)
...
- EDGE CASE (acceptable): a group_owner/group_admin with an org_membership
  ...
  111 policies ŌĆö too risky for this session; current design is sound (no leak,
...
- LeaveManagement.tsx: 2 -> showToast in LeaveRequestModal (was HALF-FIXED ŌĆö
...
(missing dependency, not a real bug). Run the migration TWICE ŌĆö second run is
the idempotency test.

## Session 24 (2026-08-18): Meeting Lifecycle Phase A ŌĆö canonical meeting subsystem foundation
...
- VideoRoom.tsx (347 lines): Jitsi Meet iframe ŌĆö the clean media-provider
  abstraction section 5 requires (kept, unchanged).
- MeetingComponents.tsx (557 lines): real MediaRecorder recording.
- transcribe-audio edge fn (232 lines): Whisper + GPT-4, JWT-verified ŌĆö the
  AI processing boundary (extend in Phase C, not now).
- events table (020): TIMESTAMPTZ + recurrence ŌĆö the scheduling foundation.

Gaps found:
- meeting-recordings storage bucket MISSING ŌĆö recording uploads silently failed.
...
- get_current_staff() RLS helper (001) ŌĆö meetings RLS rewritten to use it.
- emit_business_event telemetry (058/059) ŌĆö lifecycle RPCs emit events via
  it, NOT a new event system.
- storage.buckets pattern (030/046/1001) ŌĆö meeting-recordings bucket created.
- signing_token anon-RPC pattern (043/050) ŌĆö external guest join via token.
- update_updated_at trigger helper (007) ŌĆö reused for the new tables.
...
- meeting_participant_events (the evidence trail ŌĆö section 12): append-only.
- meeting_media (recording metadata, PRIVATE storage_path ŌĆö section 13/32).
- meeting-recordings storage bucket (PRIVATE ŌĆö never getPublicUrl).
...
- 998 TIMESTAMZT typo (TIMESTAMZT -> TIMESTAMPTZ) ŌĆö a real bug that made the
...
## Session 33 (2026-08-19): Quick Capture Multimodal ŌĆö Clip/Mic/Image (checklist item 3)
...
handlers ŌĆö multimodal capture did not exist. 1 commit, pushed to main.

### Critical latent bug found by local smoke testing
`business_events` (058) has the `business_events_updated_at` trigger
(update_updated_at ŌåÆ NEW.updated_at := NOW()) but NO updated_at column. Every
UPDATE on business_events ŌĆö including process_business_event's
`SET processed = FALSE`, which runs inside EVERY emit_business_event ŌĆö
...
- **capture_attachments table** ŌĆö attachment metadata (kind file/image/audio,
  mime, size, dimensions/duration), the captureŌåöattachment relationship
  (event_id ŌåÆ business_events), optional direct entity link
  (entity_type/entity_id), transcript (mic) + OCR result (image), status
  lifecycle (pending ŌåÆ available | failed). RLS business-scoped.
- **capture-attachments bucket (PRIVATE)** ŌĆö storage RLS keys off the path
  convention `captures/{business_id}/{attachment_id}/{file}` using a TEXT
  segment comparison (NO uuid cast ŌĆö a malformed path fails the check instead
...
- **9 RPCs** (SECURITY DEFINER + membership-guarded): create_capture_attachment
  (server-side kind/mime/size caps ŌĆö image Ōēż15MB image/*, audio Ōēż50MB
  audio/*+video/webm, file Ōēż25MB document allowlist; returns private path),
  finalize, generate_capture_attachment_url (the ┬¦32 auth gate ŌĆö never
...
- **capture-process edge fn** ŌĆö action=transcribe (Whisper) / action=ocr
...
### Frontend (AICapture.tsx ŌĆö the 3 dead buttons are now functional)
- **src/lib/captureAttachments.ts** ŌĆö pure testable helpers
  (validateCaptureFile, isMimeAllowed, shouldCompressImage, compressImage
  canvas-resize to Ōēż1920px JPEG q0.85, describeOcrAsText, captureModeFor,
  formatBytes, acceptAttrForKind) + supabase wrappers + uploadCaptureFile
  (XHR with progress events + cancel ŌĆö supabase-js exposes no progress).
- **src/components/capture/useAttachmentUploads.ts** ŌĆö upload state machine
  (validate ŌåÆ create row+path ŌåÆ XHR upload w/ progress ŌåÆ finalize ŌåÆ ready;
...
- **AttachmentTray.tsx** ŌĆö per-attachment chips: icon/preview, name, size,
  progress bar, retry-on-failure, remove; image OCR card ("What I read from
  the image" + "Use these details in the capture").
- **VoiceCapture.tsx** ŌĆö the mic production flow: permission handling
  (NotAllowed/NotFound/NotReadable each get a specific message), listening
  state (pulsing dot + timer), stop/cancel, upload ŌåÆ Whisper ŌåÆ EDITABLE
  ...
  Whisper fails ŌåÆ Web Speech API live transcription; neither ŌåÆ type
  manually. Cancel discards the uploaded row (no orphans).
- **ImageCapture.tsx** ŌĆö preview ŌåÆ validate ŌåÆ compress/resize (client canvas,
  skips <300KB + gifs) ŌåÆ confirm; shows "12.4MB ŌåÆ 890KB" before attaching.
- **AICapture integration** ŌĆö Clip/Image/Mic buttons functional. On confirm:
  ...
  UUID ŌåÆ link_capture_to_event for each ready attachment. Confirm blocked
...
"wrong" type-only path ŌĆö harmless until someone value-imports from them.
...
gate pendingŌåÆNULL, link-to-event same-business check, transcript save,
delete-returns-path).
...
the modals but create_capture_attachment errors ŌåÆ honest "Upload setup
...
## Session 35 (2026-08-19): Design + Discovery Intelligence ŌĆö parallel-session collision resolution
...
### CRITICAL LESSON ŌĆö parallel session collision (┬¦0.5 in the wild)
...
- AVENIZE-DESIGN-CONSTITUTION.md (dashed filename ŌĆö note the name)
- scripts/check_design_constitution.py + scripts/design_constitution_baseline.json
  ...
  remain, NEW violations blocked, burn-down welcomed) ŌĆö wired into
  schema-drift.yml
...
- src/components/RouteMeta.tsx: runtime public/private robots boundary for
  ...
  index,follow ŌĆö Googlebot renders JS and would see it on /app/*). Public
...
- **P2-6 + P2-7 (committed 4bf9799):** Procurement Clear-filter wired (onClearFilter prop); APISettings/Automations dead 'View setup guide ->' anchors removed (no /docs route;kept guidance text); APIKeys dead API-docs anchor -> honest span. GovernanceHub 6x + eventTracker 6x pure catch{} -> console.warn; SearchPage localStorage catch -> warn - zero pure catch{} remain in those files (grep-verified). ErrorState import exists in EmptyStates.tsx:332 for future richer error UI if wanted.
- **PENDING (standing):** Live-DB apply still needs SUPABASE_DB_URL; the edge.functions UNKNOWN local check is deno-not-installed in dev container(CI installs it.

### Session 60 continuation (2026-09-03): push + CI-green closure
- Pushed the 4 local commits (+ 3 later fixes required by the CI gate) to origin/main. Rebase needed (parallel trial-entitlement session had landed 5 commits: 20260902 trial-gate chain); regenerated contract artifacts post-rebase (5th commit — deterministic gate would have failed otherwise).
- **Two parallel-session migration-chain bugs found by the fresh-postgres migrate CI job (+ fixed):**
  1. `20260902094500_secure_intelligence_read_rpcs.sql` — 6 sql-language SECURITY DEFINER read RPCs used `s.is_active` (staff uses `active`;sql bodies resolve columns at CREATE → COALESCE(s.is_active,s.active) can't rescue unknown column). Fixed to `(s.active IS NOT FALSE)`.
  2. `20260902095000_restrict_event_and_transcript_rpc_access.sql` — forward-referencing `revoke ... reset_auth_rate_limit` (function defined+granted in zzz_auth_protocol_repair which sorts LAST→ didn't exist at that chain position);removed the contradictory revoke (owner file already grants anon+authenticated intentionally).
- **Migration-number dups (pre-existing upstream, NOT from this work):** `20260902094500` + `20260902100000` each have TWO files (parallel-session race artifacts). Flagged — renumber one per pair as a follow-up (Session-53 pattern;both likely unapplied live).
- CI: latest SHA a1d8b7e — Type Check, Lint, Unit Tests, Design Constitution, Edge Functions Type Check, Schema Drift Check, **Migrations Apply Clean, Build** all SUCCESS (the migrate job previously blocked Build).
- Deploy to Vercel: ran on the exact CI-approved commit;Deploy to Vercel ✓;E2E journey ✓;**Production contract verification ❌ (honest gate — standing live-DB/edge-fn blocker;expected. live deploy URL https://fabric-irmy5hxby-apped2.vercel.app → alias https://fabric-eta.vercel.app.
- Remaining credential-gated (unchanged): live DB apply (SUPABASE_DB_URL), edge fns deploy + secrets (PAYSTACK_SECRET_KEY, RESEND_API_KEY, EMAIL_FROM, RESEND_WEBHOOK_SECRET, APP_URL, VITE_META_PIXEL_ID)→then verify-production.sh/e2e-production.sh green→ real payment journey → ads attribution certification.
