...
- **Two parallel-session migration-chain bugs found by the fresh-postgres migrate CI job (+ fixed):**
  1. `20260902094550_secure_intelligence_read_rpcs.sql` — 6 sql-language SECURITY DEFINER read RPCs used `s.is_active` (staff uses `active`;sql bodies resolve columns at CREATE → COALESCE(s.is_active,s.active) can't rescue unknown column). Fixed to `(s.active IS NOT FALSE)`.
