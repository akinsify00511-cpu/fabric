...
4. **Executive Cockpit has a tenant/relevance dependency hidden behind RLS.** Its core transactions, deals and staff queries do not explicitly apply the current business_id filter. Although RLS may constrain results, the rendered truth should be explicitly scoped and observable rather than relying on policy behavior to define the UI dataset. Query failures are also converted to null and then downstream cards may render absence.
5. **Business Home can mislabel receivables as cash.** The Cash & Receivables card prefers an overdue/receivable metric and labels the dominant value Outstanding receivables. This is narrower than cash position and can mislead an owner about available cash.
6. **Business Home best-effort loading can hide capability failure.** Intelligence fetches deliberately swallow errors. A failed intelligence RPC can therefore become the same visible state as no available signal, rather than an explicit unavailable/error condition.
7. **Reports can render false zeros after data failure.** Reports initializes all metrics to zero and catches query errors without an error state. It also has no reporting-period selector, so aggregate values can appear current even though they cover all returned records.
8. **Cash Flow is an unbounded aggregate.** Income, expenses and net flow are calculated across all loaded cashflow records without an explicit reporting period/filter, while the surface is simply labeled Cash Flow. Load failure is logged but not surfaced.
9. **Sales Performance has an auth-timing rendering bug.** TargetsTab and CommissionsTab call their loaders from `useEffect(..., [])` even though businessId/staffId are supplied by AuthContext. If auth context is unresolved at mount, the page can permanently show the wrong empty state for the eventual authenticated business/user.
10. **Sales Performance target percentage is not denominator-safe.** Progress is calculated as actual_revenue/revenue_target without guarding a missing/zero target, so invalid numeric states can reach the rendered percentage.
11. **Monthly Performance Review can query the wrong date boundaries.** Its month generator constructs local dates and converts them through `toISOString().slice(0,10)`, which can shift date-only reporting boundaries in positive-offset locales. The visible month label can therefore disagree with the RPC period.
12. **Monthly Performance Review mixes evidence contracts.** The page labels the narrative as FACT-level, but narrative synthesis is loaded independently and best-effort after the authoritative monthly review. Its availability and evidence metadata should be explicit rather than implicitly equal to the review payload.
13. **Intelligence Hub renders generic benchmark content by default.** Market Benchmarks is always present with `salary_software_engineer_ng` as its initial metric, regardless of the user's role, industry, selected tools or need. This is generic rather than demonstrated contextual relevance.
14. **Intelligence Hub conflates error with empty.** Every failed intelligence RPC is converted to `data=null` and rendered as “No data available.” A backend failure, unavailable capability and genuine no-result state are different facts and must render differently.
15. **Dashboard empty state is comparatively strong in intent** because it explicitly avoids fabricating a sparkline when the primary metric is zero; this truthfulness standard must be applied to every chart, KPI, badge, percentage, status, activity feed, recommendation and health indicator across the product.
16. **Rendering relevance must include role, entitlement, selected tools, data freshness and workflow state.** A component should not merely render because its route exists. It should render only when its underlying capability is available and the displayed fact is applicable to the current user.
17. **Static/demo-looking content is a release defect when presented as current business state.** Examples include hard-coded metrics, decorative trend histories, fixed percentages, stale labels, unreachable tabs, placeholder actions and status labels that do not reflect persisted state.
18. **Empty/zero/unknown/loading/error states need semantic distinction.** `0`, “no data yet”, “not configured”, “not connected”, “not authorized”, “failed to load”, and “healthy” are different states and must not collapse into one visual outcome.
19. **User relevance must be tested at the rendered-output level.** Backend correctness alone cannot certify a screen if the final UI shows the wrong entity, wrong metric, wrong time range, wrong tenant, wrong status or an invented visualization.
...
- Do auth-dependent queries re-run when the authenticated business/user context becomes available?
- Are composite labels precise about the underlying fact instead of naming a broader business concept?
