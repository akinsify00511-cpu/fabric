 ...
 * - AWS SES: Set AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION env vars
 *
 * Usage:
  ...
  try {
    ...
    if (resendApiKey) {
      ...
      for (let i = 0; i < contacts.length; i += batchSize) {
        ...
        const batchResults = await sendWithResend(resendApiKey, batch, subject, html, from_email, from_name)
      ...
      for (const contact of contacts) {
        const result = await sendWithSendgrid(sendgridApiKey, contact, subject, html, from_email, from_name)
        results.push(result)
      }
    } else {
