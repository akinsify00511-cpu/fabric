...
begin
  ...
  for v_line in select value from jsonb_array_elements(p_lines) loop
    ...
    if v_account_id is null or not exists (
      ...
      raise exception 'invalid account for business';
    end if;
    ...
    if coalesce((v_line->>'debit')::numeric, 0) = 0 and coalesce((v_line->>'credit')::numeric, 0) = 0 then
      raise exception 'each journal line must have a debit or credit';
    end if;
    if coalesce((v_line->>'debit')::numeric, 0) > 0 and coalesce((v_line->>'credit')::numeric, 0) > 0 then
  ...
  insert into public.journal_entries (
    business_id, entry_number, date, reference, description, status,
    posted_by, posted_at, currency, created_at, updated_at
  ) values (
    p_business_id, v_entry_number, p_date, p_reference, p_description, 'posted',
    auth.uid(), now(), coalesce(p_currency, 'NGN'), now(), now()
  ) returning id into v_entry_id;

  for v_line in select value from jsonb_array_elements(p_lines) loop
    insert into public.journal_lines (
      business_id, journal_entry_id, account_id, debit, credit, description, currency, created_at
    ) values (
      p_business_id,
      v_entry_id,
      ...
      v_line->>'description',
      coalesce(v_line->>'currency', p_currency, 'NGN'),
      now()
    );
  end loop;
...
create constraint trigger trg_assert_posted_journal_entry_balanced
  after insert or update of status on public.journal_entries
  deferrable initially deferred
  for each row execute function public.assert_posted_journal_entry_balanced();

-- Authoritative monitoring summary. No synthetic uptime/latency values are produced.
...
as $$
  select
    count(*) filter (where m.is_active = true),
    ...
    avg(m.response_time_ms) filter (where m.is_active = true and m.response_time_ms is not null)
  from public.monitors m
  where m.business_id = p_business_id
    and exists (
      select 1 from public.staff s
      where s.business_id = p_business_id and s.user_id = auth.uid() and coalesce(s.active, s.is_active, true) = true
    );
$$;
