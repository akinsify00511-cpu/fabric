-- Fix transactional idempotency so source-specific references are durable and retry-safe.
drop index if exists public.uq_avenize_journal_entry_idempotency;
create unique index uq_avenize_journal_entry_idempotency on public.journal_entries (business_id, source_type, reference) where reference is not null;
...
declare
  ...
  v_source_type text := coalesce(nullif(btrim(p_source_type), ''), 'avenize_post_journal_entry');
  v_reference text := coalesce(nullif(btrim(p_idempotency_key), ''), nullif(btrim(p_reference), ''));
begin
  ...
  if v_reference is not null then
    select id into v_id from public.journal_entries where business_id = p_business_id and source_type = v_source_type and reference = v_reference limit 1;
  ...
  insert into public.journal_entries(business_id,date,description,status,posted_by,posted_at,reference,source_type,source_id,currency)
  values(p_business_id,p_entry_date,p_description,'posted',(select user_id from public.get_current_staff() limit 1),now(),v_reference,v_source_type,p_source_id,p_currency)
  returning id into v_id;
  insert into public.journal_lines(business_id,journal_entry_id,account_id,debit,credit,description,currency)
  select p_business_id,v_id,(x->>'account_id')::uuid,coalesce((x->>'debit')::numeric,0),coalesce((x->>'credit')::numeric,0),coalesce(x->>'description',p_description),p_currency
