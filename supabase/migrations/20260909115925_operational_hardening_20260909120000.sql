-- Operational hardening: financial writes and stock movements are atomic and fail closed.
...
begin
 ...
 for v_line in select value from jsonb_array_elements(p_lines) loop
  ...
  if v_account_id is null or not exists(select 1 from public.accounts a where a.id=v_account_id and a.business_id=p_business_id and coalesce(a.is_active,true)) then raise exception 'invalid account for business';end if;
 ...
 insert into public.journal_entries(business_id,entry_number,date,reference,description,status,posted_by,posted_at,currency,created_at,updated_at) values(p_business_id,v_entry_number,p_date,p_reference,p_description,'posted',auth.uid(),now(),coalesce(p_currency,'NGN'),now(),now()) returning id into v_entry_id;
 for v_line in select value from jsonb_array_elements(p_lines) loop insert into public.journal_lines(business_id,journal_entry_id,account_id,debit,credit,description,currency,created_at) values(p_business_id,v_entry_id,(v_line->>'account_id')::uuid,coalesce((v_line->>'debit')::numeric,0),coalesce((v_line->>'credit')::numeric,0),v_line->>'description',coalesce((v_line->>'currency'),p_currency,'NGN'),now());end loop;
 return v_entry_id;
end;$$;
...
revoke all on function public.adjust_inventory_stock(uuid,uuid,integer,text) from public;grant execute on function public.adjust_inventory_stock(uuid,uuid,integer,text) to authenticated;
