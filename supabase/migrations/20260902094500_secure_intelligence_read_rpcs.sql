...
LANGUAGE sql SECURITY DEFINER SET search_path TO 'public','pg_temp' AS $$
  ...
  WHERE km.business_id=p_business_id
    AND EXISTS (SELECT 1 FROM public.staff s WHERE s.user_id=auth.uid() AND s.business_id=p_business_id AND (s.active IS NOT FALSE))
  ORDER BY md.name;
$$;
...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'pg_catalog','public','pg_temp' AS $$
  SELECT CASE WHEN EXISTS (SELECT 1 FROM public.staff s WHERE s.user_id=auth.uid() AND s.business_id=p_business_id AND (s.active IS NOT FALSE))
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.staff s WHERE s.user_id=auth.uid() AND s.business_id=p_business_id AND (s.active IS NOT FALSE)) THEN RETURN; END IF;
