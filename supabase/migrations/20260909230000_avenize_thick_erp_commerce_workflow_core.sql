...
create table if not exists public.avenize_suppliers (
  ...
  email text, phone text, tax_id text, currency text not null default 'NGN',
...
create table if not exists public.avenize_goods_receipts (
  id uuid primary key default gen_random_uuid(), business_id uuid not null,
  purchase_order_id uuid references public.purchase_orders(id), supplier_id uuid references public.avenize_suppliers(id),
  receipt_number text not null, status text not null default 'draft', received_at timestamptz,
  received_by uuid, notes text, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
...
create table if not exists public.avenize_payment_allocations (
  ...
  allocated_at timestamptz not null default now(), created_at timestamptz not null default now(),
  unique(business_id,payment_reference,invoice_id)
);

create table if not exists public.avenize_fiscal_periods (
  id uuid primary key default gen_random_uuid(), business_id uuid not null, period_start date not null,
  period_end date not null, status text not null default 'open', closed_at timestamptz, closed_by uuid,
  created_at timestamptz not null default now(),
  constraint fiscal_period_dates_chk check (period_end >= period_start),
  unique(business_id,period_start,period_end)
);

create table if not exists public.avenize_cost_centers (
  id uuid primary key default gen_random_uuid(), business_id uuid not null, code text not null,
  name text not null, parent_id uuid references public.avenize_cost_centers(id),
  is_active boolean not null default true, created_at timestamptz not null default now(), unique(business_id,code)
);

create table if not exists public.avenize_country_rules (
  id uuid primary key default gen_random_uuid(), country_code text not null, rule_type text not null,
  rule_key text not null, rule_value jsonb not null default '{}'::jsonb, is_active boolean not null default true,
  ...
  unique(country_code,rule_type,rule_key)
);

create table if not exists public.avenize_workflow_executions (
  id uuid primary key default gen_random_uuid(), business_id uuid not null, workflow_key text not null,
  reference_type text, reference_id uuid, status text not null default 'pending', current_step integer not null default 0,
  ...
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
  unique(business_id,workflow_key,idempotency_key)
);

create table if not exists public.avenize_integration_events (
  id uuid primary key default gen_random_uuid(), business_id uuid not null,
  integration_id uuid references public.integrations(id), event_type text not null, direction text not null,
  idempotency_key text not null, payload jsonb not null default '{}'::jsonb, status text not null default 'pending',
  attempts integer not null default 0, last_error text, next_attempt_at timestamptz, processed_at timestamptz,
  created_at timestamptz not null default now(),
  unique(business_id,integration_id,event_type,idempotency_key)
);
...
returns numeric language sql stable security invoker set search_path = public, extensions, pg_temp as $$
  ...
  from public.avenize_inventory_ledger
  where business_id=p_business_id and product_id=p_product_id and (p_location_id is null or location_id=p_location_id);
$$;
revoke execute on function public.get_stock_balance(uuid,uuid,uuid) from anon;
...
$$;
revoke execute on function public.get_trial_balance(uuid) from anon;
