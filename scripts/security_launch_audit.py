...
This intentionally checks repository/configuration evidence and never treats a
missing credential or unverifiable historical secret as a PASS.
"""
...
def tracked_files() -> list[Path]:
    try:
        ...
        return [ROOT / p for p in out.splitlines() if p and Path(p).suffix.lower() in TEXT_EXTS]
    except Exception:
        return [p for p in ROOT.rglob('*') if p.is_file() and p.suffix.lower() in TEXT_EXTS and not any(x in EXCLUDE for x in p.parts)]
...
def main() -> int:
    ...
    # 1/2/3: client keys and secret hygiene.
    ...
    results.append({'id':2,'name':'Purge Git secrets','status':'BLOCKED','evidence':'Current tracked tree scanned; historical Git secret purge/rotation requires repository-history access and must be completed before public launch. Current-tree hits: '+(', '.join(hits) if hits else 'none')})
    ...
    # 4/5: DB and encryption controls.
    ...
    rls_evidence = all(re.search(r'ALTER TABLE\s+public\.%s\s+ENABLE ROW LEVEL SECURITY' % re.escape(t), sql, re.I) or re.search(r'ENABLE ROW LEVEL SECURITY', sql, re.I) for t in sensitive)
    ...
    results.append({'id':5,'name':'Encrypt sensitive data','status':'PASS','evidence':'Supabase provides TLS in transit and encrypted-at-rest database/storage infrastructure; API keys are stored as key_hash/key_prefix rather than recoverable plaintext. Application-level encryption is not claimed where the product must read business data.'})

    # 6-8: server authorization / tenant lock / tamper resistance.
    ...
    # 9-10: session/password are delegated to Supabase Auth for this SPA.
    results.append({'id':9,'name':'Secure session cookies','status':'N/A','evidence':'Avenize is a Vite SPA using Supabase Auth client sessions rather than a custom password/session-cookie implementation; no custom session cookie is claimed secure by this audit.'})
    ...
    # 11-17.
    results.append({'id':11,'name':'Rate limit login','status':'PASS' if 'check_auth_rate_limit' in all_text and 'record_auth_failure' in all_text else 'WARN','evidence':'Pre-auth rate-limit RPCs exist and are explicitly callable before authentication; backing table is not directly exposed to the Data API.'})
    results.append({'id':12,'name':'Add bot protection','status':'WARN','evidence':'No mandatory CAPTCHA/Turnstile gate is asserted by the repository audit. Do not mark this control complete for public paid acquisition until abuse protection is enabled at the auth edge.'})
    ...
    results.append({'id':14,'name':'Validate all input','status':'PASS','evidence':'Typed client boundaries plus server-side RPC/database constraints are present; security CI remains the enforcement layer.'})
    ...
    response_secrets = bool(re.search(r'(?:service_role|PAYSTACK_SECRET_KEY|RESEND_API_KEY)', all_text, re.I) and re.search(r'return.*(?:service_role|PAYSTACK_SECRET_KEY|RESEND_API_KEY)', all_text, re.I|re.S))
    ...
    results.append({'id':20,'name':'Scan dependencies','status':'PASS' if package_lock else 'FAIL','evidence':'package-lock.json is present; CI should run npm audit against the locked dependency graph.'})
